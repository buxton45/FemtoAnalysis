...
//________________________________________________________________________________________________________________
ThermFlowAnalysis::ThermFlowAnalysis(int aNpTBins, double apTBinSize) :
  fNpTBins(aNpTBins),
...
void ThermFlowAnalysis::BuildVnEPIngredients(ThermEvent &aEvent, double aHarmonic)
{
  ...
  for(unsigned int iPart=0; iPart<tMult; iPart++)
  {
    ...
    else if(abs(tCollection[iPart].GetEtaP()) < fEtaOI)
    {
...
//________________________________________________________________________________________________________________
void ThermFlowAnalysis::DrawFlowHarmonics()
{
  TCanvas* tFlowCan = new TCanvas("tFlowCan", "tFlowCan");
  tFlowCan->cd();
  ...
  if(fEns_Res_v3 < 0) cout << "WARNING!!!!!!!!" << endl << "fEns_Res_v3<0 so no resolution correction used!" << endl;
...
*/
  ...
  TGraphErrors* tGr_v2 = new TGraphErrors(fNpTBins, tpT, tv2EP, tpT_Err, tv2EP_Err);
    tGr_v2->SetMarkerSize(0.75);
    tGr_v2->SetMarkerColor(1);
    tGr_v2->SetMarkerStyle(20);
    tGr_v2->SetLineColor(1);
    tGr_v2->SetTitle("v_{2}{EP, |#Delta#eta| > 2.0} vs. p_{T} (|#eta|<0.8)");	

  TGraphErrors* tGr_v3 = new TGraphErrors(fNpTBins, tpT, tv3EP, tpT_Err, tv3EP_Err);
    tGr_v3->SetMarkerSize(0.75);
    tGr_v3->SetMarkerColor(2);
    tGr_v3->SetMarkerStyle(20);
    tGr_v3->SetLineColor(2);
    tGr_v3->SetTitle("v_{3}{EP, |#Delta#eta| > 2.0} vs. p_{T} (|#eta|<0.8)");	


  tGr_v2->GetXaxis()->SetTitle("<p_{T}>");
  tGr_v2->GetYaxis()->SetTitle("v_{n}{EP}");
