...
{
  if(aParticle->GetParticlePDGType() == kPDGLam) fLambdaCollection.push_back((ThermLambdaParticle*)aParticle);
  else if(aParticle->GetParticlePDGType() == kPDGALam) fAntiLambdaCollection.push_back((ThermLambdaParticle*)aParticle);
  else if(aParticle->GetParticlePDGType() == kPDGK0) fK0ShortCollection.push_back((ThermLambdaParticle*)aParticle);
...
void ThermEvent::FindFatherandLoadDaughter(ThermParticle* aDaughterParticle)
{
  ...
  vector<ThermLambdaParticle*> tFatherCollection; //just a copy, when I load, I must use the actual object
...
void ThermEvent::MatchDaughtersWithFathers(ParticlePDGType aDaughterType)
{
  ...
  {
    if(IsDaughterOfInterest(tDaughterCollection[i]->GetFatherPDGType())) FindFatherandLoadDaughter(tDaughterCollection[i]);
  }
