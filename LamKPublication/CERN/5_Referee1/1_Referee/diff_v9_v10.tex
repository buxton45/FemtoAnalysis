...
%DIF DEL LamKPublication_v9_BeforeAddressingReferee1.tex   Mon Apr 13 19:01:01 2020
%DIF ADD LamKPublication_v10.tex                           Tue Oct 27 19:19:23 2020
...
\begin{titlepage}
%
\DIFdelbegin %DIFDELCMD < \PHyear{2019 }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \PHyear{2020 }
\DIFaddend \PHnumber{XXX}      % required, will be obtained from PH
...
The extracted scattering parameters indicate that the strong force is repulsive in the \LamKchP interaction and attractive in the \LamKchM \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{interaction.
The data hint that the }\DIFaddend \LamKs \DIFdelbegin \DIFdel{interactions}\DIFdelend \DIFaddbegin \DIFadd{interaction is attractive, however the uncertainty of the result does not permit such a decisive conclusion}\DIFaddend .
...
Extensive studies with the THERMINATOR 2\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Chojnacki:2011hb} }%DIFAUXCMD
}\DIFaddend event generator are performed to account for both non-femtoscopic backgrounds as well as contributions from residual correlations induced by feed-down from particle decays.
...
\begin{figure}[h]
  ...
  \DIFaddbeginFL \DIFaddFL{Statistical (lines) and systematic (boxes) uncertainties are shown for the experimental data.
  }\DIFaddendFL Results are shown for \LamKchP (left), \LamKchM (middle), and \LamKs (right).
...
%  \includegraphics[width=\linewidth]{\ResultsDirBaseLamKs\SaveNameModLamKs/canKStarCfwFits_CombineConj_AllAn.pdf}
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\linewidth]{\ResultsDirBaseLamKch\SaveNameModLamKch/canKStarCfwFits_CombineConj_AllAn_LabelLines.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\linewidth]{\ResultsDirBaseLamKch\SaveNameModLamKch/canKStarCfwFits_CombineConj_AllAn_LabelLines_wErrBands.pdf}
  \DIFaddendFL \caption[\LamK data with fits]
  {
  ...
  \DIFaddbeginFL \DIFaddFL{The shaded region represents the (combined statistical and systematic) uncertainty of the final fit.
 }\DIFaddendFL }
...
\begin{figure}[h]
  ...
  \DIFaddbeginFL \DIFaddFL{In the figures, lines represent statistical uncertainties, while boxes represent systematic uncertainties.  
  }\DIFaddendFL (Left) The scattering parameters, $\Im f_{0}$ and $\Re f_{0}$, together with $d_{0}$ to the right, for the \LamKchP (circles), \LamKchM (squares) and \LamKs (stars) systems.  
...
Therefore, the femtoscopic signals from this analysis demonstrate that the strong interaction acts repulsively in the \LamKchP system \DIFdelbegin \DIFdel{, and acts }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend attractively in the \LamKchM \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{system.
The analysis suggests that the }\DIFaddend \LamKs \DIFdelbegin \DIFdel{systems}\DIFdelend \DIFaddbegin \DIFadd{interaction is attractive, however the uncertainty of the result does not permit a definite conclusion}\DIFaddend .
...
\begin{figure}[h]
  ...
  \DIFaddbeginFL \DIFaddFL{Statistical (lines) and systematic (boxes) uncertainties are shown. 
  }\DIFaddendFL }
...
\begin{figure}[h!]
  ...
  \DIFaddbeginFL \DIFaddFL{Statistical (lines) and systematic (boxes) uncertainties are shown.
}\DIFaddendFL The $C_{00}$ component is similar to the one-dimensional correlation functions typically studied, and probes the overall size of the source.
...
These scattering parameters indicate that the strong force is repulsive in the \LamKchP interaction and attractive in the \LamKchM \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{interaction, and suggest that the interaction is also attractive in the }\DIFaddend \LamKs \DIFdelbegin \DIFdel{interactions}\DIFdelend \DIFaddbegin \DIFadd{system}\DIFaddend .
...
\begin{figure}[h!]
  ...
  \DIFaddbeginFL \DIFaddFL{Where experimental data are shown, lines represent statistical uncertainties, while boxes represent systematic uncertainties.
  }\DIFaddendFL (a) the one-dimensional correlation function from THERMINATOR 2 together with the experimental data.
...
\DIFdelbegin \DIFdel{Only }\DIFdelend \DIFaddbegin \DIFadd{The real component of the scattering length describes the effect of the strong interaction, and is very narrow in }\kstar \DIFadd{(}\kstar \DIFadd{$\lesssim$ 100 MeV/$c$).
The imaginary component of the scattering length accounts for inelastic scattering channels, and produces a wide (hundreds of MeV/$c$) negative correlation which goes to zero at }\kstar\DIFadd{= 0 MeV/$c$.
The interplay of these two components determines the final shape of the correlation function; in the case of Fig.~\ref{fig:LamKchP_ThermSources_VaryMuOut}, although both $\Re(f_{0})$ and $\Im(f_{0})$ lead to a suppression, the combination of these distinct contributions results in a rise of the correlation function at low }\kstar \DIFadd{(while still remaining less than unity).
For the fit, only }\DIFaddend the extracted one-dimensional source size is of interest here, so the $\lambda$ parameter is also fixed at unity.
...
\begin{figure}[h]
  ...
  {
  (Color online) Spherical harmonics components (left) $C_{00}$ and (right) $\Re C_{11}$ of the \LamKchP correlation function for the 0--10\% centrality interval shown with results from the THERMINATOR 2 simulation implemented with different shifts in the outward direction, $\mu_{\mathrm{out}}$, as described in the text\DIFaddbeginFL \DIFaddFL{.
  Statistical (lines) and systematic (boxes) uncertainties are shown for the experimental data}\DIFaddendFL .
  }
