...
{\color{red}{The fit assumes a common radius and $\lambda_{\mathrm{Fit}}$ parameter for each centrality bin, shared across all \LamK pair systems.}}
The femtoscopic correlations result from strong final-state interactions, and are fit with a parametrization allowing for both the characterization {\color{blue}{of}} the pair emission source and the measurement of the scattering parameters for the particle pairs.
...
{\color{red}{In the experimental data, a striking difference is observed in pairs with low relative momenta ($k^{*}\lesssim$ 100 MeV) between the $\Lambda$K$^{+}$ and $\Lambda$K$^{-}$ correlations, and the $\Lambda\mathrm{K^{0}_{S}}$ system exhibits features between the two.}}
{\color{red}{The $\Lambda$K$^{+}$ system exhibits a negative real component of the scattering parameter ($\Re f_{0}$), while those of the $\Lambda$K$^{-}$ and $\Lambda\mathrm{K^{0}_{S}}$ are positive, although the magnitude of that of the $\Lambda\mathrm{K^{0}_{S}}$ is smaller than that of the $\Lambda$K$^{-}$.}}
{\color{red}{The results suggest an effect arising from different quark-antiquark interactions between the pairs ($\rm s\bar{s}$ in $\Lambda$K$^{+}$ and $\rm u\bar{u}$ in $\Lambda$K$^{-}$), or from different net strangeness for each system (S=0 for $\Lambda$K$^{+}$, and S=$-2$ for $\Lambda$K$^{-}$).}}
{\color{blue}{The extracted scattering parameters indicate that the strong force is repulsive in the \LamKchP interaction and attractive in the \LamKchM and \LamKs interactions.}}
{\color{blue}{The results suggest an effect arising from different quark-antiquark interactions between the pairs ($\rm s\overline{s}$ in $\Lambda$K$^{+}$ and $\rm u\overline{u}$ in $\Lambda$K$^{-}$), or from different net strangeness for each system (S=0 for $\Lambda$K$^{+}$, and S=$-2$ for $\Lambda$K$^{-}$).}}
...
With this method, {\color{red}{two(or many)-particle}} {\color{blue}{two (or many)-particle}} relative-momentum correlation functions are used to connect the final-state momentum distributions to the space-time distributions of particle emission at freeze-out.  
...
{\color{red}{In many pair systems, the contributions to the correlation function from quantum statistics and/or the Coulomb interaction overwhelm that of the strong interaction, making it difficult to extract scattering information.}}
...
Scattering parameters for similar systems are also very limited; past studies of kaon-proton scattering revealed the strong force is attractive in the K$^{-}$p interaction, and repulsive in that of the K$^{+}$p {\color{blue}{\cite{Humphrey:1962zz, Hadjimichef:2002xe, Ikeda:2012au}}}.
...
For each PID method, a value ({\color{red}{$N\sigma$}} {\color{blue}{$N_{\sigma}$}}) was assigned to each track denoting the number of standard deviations between the measured track information and calculated values.  
This procedure was repeated for four ``particle species hypotheses''--- electron, pion, kaon, and proton---, and for each hypothesis a different {\color{red}{$N\sigma$}} {\color{blue}{$N_{\sigma}$}} value was obtained per detector.
...
PID was performed using both the TPC and TOF detectors via the {\color{red}{$\mathrm{N}\sigma$}} {\color{blue}{$N_{\sigma}$}} method. 
The {\color{red}{$\mathrm{N}\sigma$}} {\color{blue}{$N_{\sigma}$}} cuts become tighter with increasing momentum to reduce contamination within the samples, as the \Kpm signals begin to overlap more significantly with those from other particles, particularly e$^{\pm}$ and $\pi^{\pm}$.
...
The purity of the \Kpm collections{\color{blue}{, $P_{\mathrm{K}^{\pm}}$,}} was estimated to be approximately 97\% from a Monte-Carlo (MC) study based on HIJING \cite{PhysRevD.44.3501} simulations using GEANT3 \cite{Brun:1994aa} to model particle transport through the ALICE detectors. 
{\color{blue}{For a more detailed estimate of the \Kpm purity from an analysis employing similar cuts, see Ref.\ \cite{Acharya:2017qtq}.}}
...
{\color{red}{\LamALam and \Ks particles are electrically neutral, and cannot be directly detected, but must instead be reconstructed through detection of their decay products, or daughters.}} 
{\color{red}{In general, particles which are topologically reconstructed in this fashion are called \Vz particles.}}
{\color{red}{The decay channel \Lam $\rightarrow$ p$\pi^{-}$ was used for the identification of \Lam hyperons (and, similarly the charge-conjugate decay for the \ALam identification), and \Ks $\rightarrow$ $\pi^{+}\pi^{-}$ for the identification of \Ks mesons.}}
{\color{blue}{\LamALam and \Ks particles are reconstructed through their weak decays: \Lam $\rightarrow$ p$\pi^{-}$ (\ALam $\rightarrow \pi^{+}\overline{\mathrm{p}}$) and \Ks $\rightarrow$ $\pi^{+}\pi^{-}$.}}
{\color{blue}{The obtained candidates are denominated as \Vz particles.}}
The main cuts used are shown in Tables \ref{tab:LamCuts} and \ref{tab:K0sCuts}.

{\color{red}{To construct a \Vz particle, the charged daughter tracks must first be found.}}
...
If a daughter is found to be shared between \Vz candidates, {\color{red}{the candidate with the smallest DCA to the primary vertex is kept, while the others are removed}} {\color{blue}{only the candidate with the smallest DCA to the primary vertex is kept}}.
...
The \Lam and \ALam purities are estimated to be {\color{blue}{$P_{\Lambda(\overline{\Lambda})} \approx$}} $95\%$, and that of the \Ks is {\color{blue}{$P_{\mathrm{K^{0}_{S}}} \approx$}} $98\%$.
...
\begin{figure}[htp]
  ...
  %%----overall caption----
  \caption{
  {\color{blue}{(Color online)}} Invariant mass (\minv) distribution of p$\pi^{+}$ pairs showing the \Lam peak \ref{fig:Purity:a}, and of $\pi^{+}\pi^{-}$ pairs showing the \Ks peak \ref{fig:Purity:b}, for \Vz candidates.  
  The bottom panels are zoomed to show the background with fit.  
  The vertical {\color{red}{green}} {\color{blue}{dashed}} lines represent the \minv cuts used in the analyses, the {\color{red}{red}} vertical {\color{blue}{dotted}} lines delineate the region over which the background was fit, and the {\color{red}{blue}} {\color{blue}{dash-dotted}} line shows the background fit.
  }  
  \label{fig:Purity}
\end{figure}
...
{\color{red}{Equation \ref{eqn:KooninPrattEqn} reveals the limitations of femtoscopy; at best, it can probe the distribution of relative positions of particles with identical velocities and total momentum $\mathbf{P}$ as they move in an asymptotic state \cite{Lisa:2005dd}.}}
{\color{red}{Therefore, the entire size of the source is not measured, but rather the ``regions of homogeneity'' \cite{Akkelin:1995gh}.}}
...
Typically, $B(k^{*})$ is obtained {\color{red}{by forming}} {\color{blue}{using}} mixed-event pairs \cite{Kopylov:1974th}, i.e. particles from a given event are paired with {\color{red}{particles from $\rm N_{mix}$ other events, and these pairs are then binned in \kstar}} {\color{blue}{those from another event}}.
...
For this analysis, the typical mixed-event method is utilized, and each event is mixed with {\color{red}{$\rm N_{mix} =$ 5}} {\color{blue}{five}} others for the reference distribution construction.
...
{\color{red}{After the event binning, a vertex correction is applied to each event, in which the primary vertex position is subtracted from the TPC track positions for each particle in the event.}}
{\color{red}{Subsequently, the tracks in an event are all measured relative to the event vertex, instead of the center of the TPC.}}
{\color{red}{This effectively gives each event the same primary vertex position, which is important for the implementation of the average separation cut in mixed-event pairs.}}
...
The $k_{\mathrm{T}}$-dependence of the three \LamK combinations {\color{red}{should be the same}} {\color{blue}{is comparable}}, so an integrated analysis is acceptable.
...
{\color{red}{Within the model, the (non-symmetrized) two-particle wave function is expressed as a superposition of a plane wave and diverging spherical wave:}
\begin{equation}
\begin{aligned}
...
{\color{blue}{Within the model, the (non-symmetrized) two-particle wave function is expressed as a superposition of a plane wave and diverging spherical wave, and the complex scattering amplitude, $f^{S}(k^{*})$, is evaluated via the effective range approximation}}
\begin{equation}
\begin{aligned}
...
\end{equation}
where $f^{S}_{0}$ is the complex s-wave scattering length, {\color{red}{and}} $d^{S}_{0}$ is the effective range of the interaction {\color{blue}{, and $S$ denotes the total spin of the particular pair}}.
{\color{blue}{The sign convention is such that a positive real component of the scattering length, $\Re f_{0}$, represents an attractive interaction, while a negative $\Re f_{0}$ represents a repulsion.}}
...
where $\Re f^{S}(k^{*})$ and $\Im f^{S}(k^{*})$ denote the real and imaginary parts of the complex scattering length, respectively, and $F_{1}$ and $F_{2}$ are the analytic functions{\color{blue}{.}}
...
{\color{red}{However, in practice, obtaining a pure sample of primary \LamK pairs is impossible to achieve; some of the selected particles originate as decay products from other resonances, and some of the final pairs contain a misidentified member.}}
{\color{blue}{However, in practice some of the selected particles originate as decay products from other resonances, and some of the final pairs contain a misidentified member.}}
...
{\color{blue}{The contributions from fake pairs, which contain at least one misidentified member, are assumed to average to unity, in which case they simply attenuate the femtoscopic signal.}}
...
{\color{red}{For this analysis, a portion of the selected \Lam hyperons decay from $\Sigma^{0}$, $\Xi^{0}$, $\Xi^{-}$ and $\Sigma^{*(+,-,0)}(1385)$ parents, and some of the K mesons decay from K$^{*(+,-,0)}(892)$ parents.}}
{\color{red}{The contribution from fake pairs, which contain a misidentified member, is assumed to average to unity, in which case it leads to an attenuation of the femtoscopic signal.}} 
{\color{blue}{As described in the following, the main sources of residual correlations in the \LamK systems result from \Lam hyperons which have decayed from $\Sigma^{0}$, $\Xi^{0}$, and $\Xi^{-}$ parents.}}

The finally measured correlation function is a combination of the genuine \LamK correlation with contributions from {\color{red}{residual feed-down and misidentified particles}} {\color{blue}{resonances and impurities \cite{Kisiel:2014mma}}}
\begin{equation}
\begin{aligned}
\label{eqn:CfwRes} 
...
\end{aligned} 
\end{equation}
where the \LamK term represents the genuine \LamK correlation, and the $ij$ terms denote the contributions from residual feed-down and {\color{red}{possible}} impurities.
More specifically, $C_{ij}(k^{*}_{\Lambda\mathrm{K}})$ is the correlation function {\color{red}{between parents of particle species $i$ and $j$, expressed in terms of the relative momentum of the observed daughter \LamK pairs}} {\color{blue}{of the parent system expressed in terms of the relative momentum of the daughter \LamK pair}}.  
The $\lambda_{ij}$ parameters serve as weights dictating the relative strength of {\color{red}{the parent contribution to the daughter pair}} {\color{blue}{each component's contribution to the observed signal}}, and are normalized to unity (i.e. $\sum_{ij} \lambda_{ij} = 1$, where $ij$ includes also the primary \LamK component).
The individual $\lambda_{ij}$ are fixed (and whose values can be found in Table \ref{tab:LambdaValues_3Res}), but the parameter $\lambda_{\mathrm{Fit}}$ {\color{blue}{in Eq.\ \ref{eqn:CfwRes}}} is left free.
{\color{red}{The $\lambda_{\mathrm{Fit}}$ parameter serves as an common factor shared by all contributors, as shown in Eq. \ref{eqn:CfwRes}.}}
...
{\color{red}{In order to obtain the parent correlation function expressed in the relative momentum of the daughter pair, one must use a transform matrix.}}
{\color{red}{The transform matrix describes the decay kinematics of the parent system into the daughter, and maps the \kstar of the parent pair onto that of the daughter.}}
{\color{red}{Using this matrix, the transformed residual correlation function can be obtained:}}
{\color{blue}{To obtain the parent correlation function expressed in the relative momentum of the daughter pair, a transform matrix is utilized}}
\begin{equation}
...
{\color{red}{The transform matrix is generated with the THERMINATOR 2 \cite{Chojnacki:2011hb} simulation. }}
{\color{red}{It is formed for a given parent pair, $ij$, by taking all \LamK pairs originating from $ij$, calculating the relative momentum of the parents ($k^{*}_{ij}$) and daughters ($k^{*}_{\Lambda\mathrm{K}}$), and filling a two-dimensional histogram with the values.}}
{\color{red}{The transform matrix is essentially an unnormalized probability distribution mapping the \kstar of the parent pair to that of the daughter pair when one or both parents decay.}}
{\color{red}{As previously stated, the $\lambda$ parameters dictate the strength of the parent contribution to the daughter pair.  }}
{\color{red}{Therefore, the $\lambda$ parameter for parent system AB can be estimated as the total number of $\Lambda$K pairs in our experimental sample originating from AB (N$_{AB}$) divided by the total number of $\Lambda$K pairs (N$_{Total}$):
\begin{equation}
...
{\color{blue}{where $T(k^{*}_{ij},k^{*}_{\Lambda\mathrm{K}})$ is the transform matrix, which is generated with the THERMINATOR 2 \cite{Chojnacki:2011hb} simulation. 
...
{\color{blue}{The $\lambda_{ij}$ parameters dictate the relative strength of each contribution to the correlation function, and can be estimated using the THERMINATOR 2 and HIJING simulations.
...
{\color{blue}{The number of \LamK pairs is then estimated as the product of the yield with the reconstruction efficiency, $N_{ij} = N_{ij}^{\scaleto{THERM}{3pt}}RE_{ij}^{\scaleto{HIJING}{3pt}}$.}}
...
To estimate {\color{red}{this}} {\color{blue}{the}} $\lambda_{\mathrm{Fakes}}$ value, the number of fake pairs is assumed to be equal to the total number of {\color{red}{pairs multiplied by the \Lam purity (i.e. assuming perfect purity for the kaons); or, more simply, $\lambda_{\mathrm{Fakes}}$ = 1.0 $-$ Purity(\Lam)}} {\color{blue}{simulated pairs multiplied by $(1-PP_{\Lambda\mathrm{K}})/PP_{\Lambda\mathrm{K}}$, where $PP_{\Lambda\mathrm{K}}$ is the \LamK pair purity, estimated as the product of the two single-particle purities ($PP_{\Lambda\mathrm{K}} = P_{\Lambda}P_{\mathrm{K}}$).}}
{\color{blue}{More simply, this amounts to $\lambda_{\mathrm{Fakes}} = 1.0-PP_{\Lambda\mathrm{K}}$.}}
{\color{red}{For both of these contributors (``Other'' and ``Fakes''), it is assumed that these correlations average to unity, and therefore do not contribute to the final correlation function.}}
{\color{blue}{The correlations in both of these categories (``Other'' and ``Fakes'') are assumed to average to unity, and pairs in these categories therefore only contribute by attenuating the signal. }}
...
{\color{red}{Smearing of the momentum typically will result in a suppression and broadening of the signal.}}
...
{\color{blue}{The}} THERMINATOR 2 simulation has been shown to reproduce the background features in a $\pi$K analysis \cite{Kisiel:2017}. 
{\color{red}{
...
Figure \ref{fig:BgdswTHERM} shows the THERMINATOR 2 simulation ({\color{red}{gold}} {\color{blue}{open triangles}}) together with experimental data ({\color{red}{red, blue, or black}} {\color{blue}{closed circles}}).  
The figure also shows a {\color{red}{6$^{th}$-order}} {\color{blue}{6$^{\mathrm{th}}$-order}} polynomial fit to the simulation ({\color{red}{gold}} {\color{blue}{dashed curves}}), as well as the fit polynomial scaled to match the data ({\color{red}{red, blue, or black}} {\color{blue}{solid curves}}).
...
The form of each polynomial {\color{red}{was}} {\color{blue}{is}} set before use with the experimental data, by fitting to the THERMINATOR 2 simulation, shown in Fig.\ \ref{fig:BgdswTHERM}.
{\color{red}{
...
{\color{blue}{The extracted polynomial is adjusted to best describe the data by introducing a scale factor and a vertical shift, both of which are determined by fitting to data in the region $0.32 < k^{*} < 0.80$ GeV/$c$; during the fit of the low-\kstar signal region, the background is fixed.}}
...
\begin{figure}[h]
  ...
  {
  {\color{blue}{[NEW FIGURE]}} {\color{blue}{(Color online)}} THERMINATOR 2 simulation ({\color{red}{gold}} {\color{blue}{open triangles}}) together with experimental data ({\color{red}{red, blue, or black}} {\color{blue}{closed circles}}).  
  Results are shown for \LamKchP (left), \LamKchM (middle), and \LamKs (right).
  A {\color{red}{6$^{th}$-order}} {\color{blue}{$6^{\mathrm{th}}$-order}} polynomial fit to the simulation is shown as a {\color{red}{solid gold line}} {\color{blue}{dashed curve}}.  
  This polynomial is scaled to match the experimental data.  
  The polynomial fit with scale factor applied is drawn {\color{red}{in a color matching the experimental data (red, blue, black)}} {\color{blue}{as a solid curve}}.
  }
...
{\color{blue}{The parameters included in the generation of a correlation function are: $\lambda_{\mathrm{Fit}}$, $R$, $f_{0}$ ($\Re f_{0}$ and $\Im f_{0}$ separately), $d_{0}$, and normalization $\mathcal{N}$.}}
...
All correlation functions {\color{red}{were}} {\color{blue}{are}} normalized in the range 0.32 $< k^{*} <$ 0.40 {\color{red}{GeV/c}} {\color{blue}{GeV/$c$}}, and fit in the range 0.0 $< k^{*} <$ 0.30 {\color{red}{GeV/c}} {\color{blue}{GeV/$c$}}.
...
The non-femtoscopic backgrounds are modeled using the THERMINATOR 2 simulation {\color{red}{for the \LamKpm analyses, and with a linear form for the \LamKs system}}, as described in Sec.\ \ref{NonFlatBackground}.
{\color{red}{In general, corrections are applied to the fit function, the raw data is never touched.}}
{\color{blue}{A log-likelihood fit function is used as the statistic quantifying the quality of the fit \cite{Lisa:2005dd}.}}
...
{\color{red}{In order to understand the systematic uncertainties of the data, the analysis code was run many times using slightly different values for a number of important cuts, and the results were compared.}}
...
The cuts included in the systematic study, as well as the values used in the variations, are shown in {\color{red}{Tab. \ref{tab:LamK0sSystematics} (\LamKs) and Tab. \ref{tab:LamKchSystematics} (\LamKpm)}} {\color{blue}{Tab.\ \ref{tab:LamKSystematics}}}.  
...
{\color{red}{As with the systematic errors on the data, this was performed for all varied cut values.}}
{\color{red}{Additionally, a systematic analysis was done on the fit method through varying the \kstar fit range, as well as varying the modeling of the non-femtoscopic background.}}
{\color{blue}{Additionally, for the extracted fit parameters, a systematic analysis was done on the fit method through varying the \kstar fit range, as well as varying the modeling of the non-femtoscopic background.}}
The choice of \kstar fit range was varied by $\pm$ 25\%. 
As previously stated, the non-femtoscopic backgrounds are modeled {\color{red}{using the THERMINATOR 2 simulation for the \LamKpm analyses, and with a linear form for the \LamKs system.}} {\color{blue}{with a polynomial fit to the THERMINATOR 2 simulation, scaled to match the data}}.
...
{\color{red}{Additionally, the backgrounds of all systems were modeled with a polynomial fit to the THERMINATOR simulation, scaled to match the data.}}
...
All {\color{red}{analyses were}} {\color{blue}{six \LamK systems (\LamKchP, \ALamKchM, \LamKchM, \ALamKchP, \LamKs, \ALamKs) are}} fit simultaneously across all centralities, with a single radius and normalization {\color{red}{$\lambda$}} {\color{blue}{$\lambda_{\mathrm{Fit}}$}} parameter for each centrality bin.
Scattering parameters ($\Re f_{0}$, $\Im f_{0}$, $d_{0}$) {\color{red}{were}} {\color{blue}{are}} shared between pair-conjugate systems, but assumed unique between the different \LamK charge combinations (i.e. a parameter set describing the \LamKchP \& \ALamKchM system, a second set describing the \LamKchM \& \ALamKchP system, and a third for the \LamKs \& \ALamKs system).
Each correlation function {\color{red}{received}} {\color{blue}{receives}} a unique normalization parameter.
The fits {\color{red}{were}} {\color{blue}{are}} corrected for finite momentum resolution effects, non-femtoscopic backgrounds, and residual correlations resulting from the feed-down from resonances.
...
The {\color{red}{black solid}} {\color{blue}{dotted}} curve shows the primary (\LamK) contribution to the fit (i.e. $1 + \lambda'_{\Lambda\mathrm{K}}C_{\Lambda\mathrm{K}}(k^{*}_{\Lambda\mathrm{K}})$ in Eq.\ \ref{eqn:CfwRes}), the {\color{red}{green}} {\color{blue}{dashed}} curve shows the fit to the non-femtoscopic background {\color{red}{(with a polynomial fit to the THERMINATOR 2 simulation for the \LamKpm systems, and a linear form fit to the data for the \LamKs)}}, and the {\color{red}{purple}} {\color{blue}{solid}} curve shows the final fit after all corrections have been applied.
{\color{red}{The extracted fit values with uncertainties are printed as (fit value) $\pm$ (statistical uncertainty) $\pm$ (systematic uncertainty).}}
...
\begin{figure}[h]
  ...
  {
  {\color{blue}{[NEW FIGURE]}} {\color{blue}{(Color online)}} Fit results for the \LamKchP and \ALamKchM data.
...
\begin{figure}[h!]
  ...
  {
  {\color{blue}{[NEW FIGURE]}} {\color{blue}{(Color online)}} Fit results for the \LamKchM and \ALamKchP data.
...
\begin{figure}[h!]
  ...
  {
  {\color{blue}{[NEW FIGURE]}} {\color{blue}{(Color online)}} Fit results for the \LamKs and \ALamKs data.
...
\begin{figure}[h]
  ...
  {
  {\color{blue}{[NEW FIGURE]}} {\color{blue}{(Color online)}} Extracted fit parameters for all of the \LamK systems.  
  [Left]: $\Im f_{0}$ vs. $\Re f_{0}$, together with $d_{0}$ to the right {\color{blue}{for the \LamKchP (circles), \LamKchM (squares) and \LamKs (triangles) systems}}.  
  [Right]: $\lambda$ vs. Radius for the 0--10\% ({\color{red}{blue}} {\color{blue}{circles}}), 10--30\% ({\color{red}{green}} {\color{blue}{squares}}), and 30--50\% ({\color{red}{red}} {\color{blue}{triangles}}) centrality bins.  
  In the fit, all \LamK systems share common radii.
  {\color{red}{The color scheme used in the panel are to be consistent with those in Fig. \ref{fig:mTScalingOfRadii_3Res}.}}
  The {\color{red}{cyan}} {\color{blue}{cross}} ([A] = Ref.\ \cite{Liu:2006xja}) and {\color{red}{magenta}} {\color{blue}{X}} symbols ([B] = Ref.\ \cite{Mai:2009ce}) points show theoretical predictions made using chiral perturbation theory.
  }
...
For all {\color{red}{studied}} {\color{blue}{\LamK}} systems, {\color{red}{a}} positive imaginary {\color{red}{part, $\Im(f_{0})$,}} {\color{blue}{parts}} of the scattering lengths{\color{blue}{, $\Im(f_{0})$,}} are extracted. 
...
{\color{red}{Each of the three systems has scattering parameters unique from the others.}} 
...
{\color{red}{A positive $\Re(f_{0})$ (\LamKchM, \LamKs) signifies that the effect of the strong force is attractive, while a negative $\Re(f_{0})$ (\LamKchP) signifies a repulsion.}}
{\color{blue}{As is the usual convention in femtoscopy, a positive $\Re(f_{0})$ (\LamKchM, \LamKs) signifies that the effect of the interaction is attractive, while a negative $\Re(f_{0})$ (\LamKchP) signifies a repulsive interaction.}}
Past studies of kaon-proton scattering found the K$^{-}$--p interaction to be attractive, and that of the K$^{+}$--p to be repulsive {\color{blue}{\cite{Humphrey:1962zz, Hadjimichef:2002xe, Ikeda:2012au}}}.
...
\begin{figure}[h]
  ...
  {
  {\color{blue}{[NEW FIGURE]}} {\color{blue}{(Color online)}} Extracted fit $R_{\mathrm{inv}}$ parameters as a function of pair transverse mass (\mt) for various pair systems over several centralities. 
...
It was found that \cite{Kisiel:2014upa}, even in the presence of good global \mt-scaling for the three-dimensional radii in the {\color{red}{LCMS frame}} {\color{blue}{Longitudinally Co-Moving System (LCMS)}}, a particle species dependence will exist for the $R_{\mathrm{inv}}$ measured in the {\color{red}{PRF}} {\color{blue}{Pair Rest Frame (PRF)}}, due to trivial kinematic reasons.
...
{\color{red}{In general, each single-particle source will have its own size, shape, and space-time position within the produced medium, which is unique from its paired partner.}}
...
More specifically, the hydrodynamic response of the system confines higher-\mt particles to smaller homogeneity regions, {\color{red}{also}} also pushes their average emission points further in the ``out" direction \cite{Retiere:2003kf}.
...
In addition to this, non-identical particle studies are able to measure the relative emission shifts, the first moments of the emission function {\color{blue}{\cite{Kisiel:2009eh}}}.
...
\begin{figure}[h!]
  ...
  {
  {\color{blue}{(Color online)}} $C_{00}$ (left) and $\Re C_{11}$ (right) components of a spherical harmonic decomposition of the \LamKchP correlation function for the 0--10\% centrality bin.  
...
{\color{red}{A striking difference in the \LamKchP and \LamKchM correlation functions is observed, which is}} {\color{blue}{Striking differences are observed in the \LamKchP, \LamKchM, and \LamKs correlation functions, which are}} reflected in the unique set of scattering parameters extracted for each.
{\color{red}{The \LamKs systems is somewhat intermediate to these two cases.}}
{\color{red}{
...
With the Stavinskiy method, mixed-event pairs are not used for the reference distribution; instead, same-event pseudo-pairs, formed by rotating one particle in a real pair by 180$^\circ$ in the transverse plane, are used.  
This rotation rids the pairs of any femtoscopic correlation, while maintaining correlations due to elliptic flow (and other suitably symmetric contributors).
{\color{blue}{Care needs to be taken in treating the pseudo-pairs exactly like the real pairs; e.g. the pseudo-pairs should be exposed to the same pair cuts used in the analysis on the real pairs.}}
...
{\color{blue}{The figure shows the Stavinskiy method does a very good job of ridding the \LamKchP correlations of their non-femtoscopic backgrounds.}}
...
Fig.\ \ref{fig:LamKchP_StdThermSources} shows results from the THERMINATOR 2 event generator for an impact parameter of {\color{red}{b = 2}} {\color{blue}{$b = 2$ fm}}.
As THERMINATOR {\color{blue}{2}} does not include any final state effects, the femtoscopic correlation was introduced by assuming a set of scattering parameters ($\Re f_{0}, \Im f_{0}, d_{0}$) = ($-1.16, 0.51, 1.08$) and weighting the signal distribution (numerator pairs) with the modulus squared of the two-particle wave function, $|\Psi|^{2}$.
...
In Figure \ref{fig:LamKchP_ThermSources_VaryMuOut}, results are shown for the case of {\color{red}{$\mu_{out}$ = 1 fm, $\mu_{out}$ = 3 fm, and $\mu_{out}$ = 6 fm}} {\color{blue}{$\mu_{\mathrm{out}}$ = 1 fm, $\mu_{\mathrm{out}}$ = 3 fm, and $\mu_{\mathrm{out}}$ = 6 fm}}.
...
The figure demonstrates that as the separation {\color{red}{$\mu_{out}$}} {\color{blue}{$\mu_{\mathrm{out}}$}} increases, so do the extracted femtoscopic radii.
...
\begin{figure}[h!]
  ...
  {
  {\color{blue}{(Color online)}} Extracted radius when performing a simple fit on simulation from THERMINATOR 2, along with the spatio-temporal characteristics generated by the simulation.
  }
...
\begin{figure}[h]
  ...
  {
  {\color{blue}{(Color online)}} Probing the effect of varying the source shift in the outward direction, {\color{red}{$\mu_{\mathrm{Out}}$}} {\color{blue}{$\mu_{\mathrm{out}}$}}, within the THERMINATOR 2 framework.  
  To achieve this, particle pairs are formed from the simulation, but with altered spatial characteristics achieved by drawing the out, side, and long components from {\color{red}{pre-determined}} {\color{blue}{predetermined}} Gaussian distributions.  
  ...
  Moving from top to bottom, {\color{red}{$\mu_{\mathrm{Out}}$}} {\color{blue}{$\mu_{\mathrm{out}}$}} increase from 0 to 6 fm, the effect of which clearly increases the effective radius extracted in the fit.
  }
