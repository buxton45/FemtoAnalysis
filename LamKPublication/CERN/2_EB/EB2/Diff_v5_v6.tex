...
For a more detailed estimate of the \Kpm purity from an analysis employing similar methods, see {\color{red}{Ref.}}~\cite{Acharya:2017qtq}.


\begin{table}[htbp]
 \centering
 \caption{{\color{red}{Charged kaon (\Kpm) selection criteria}} {\color{blue}{Selection criteria for \Kpm mesons}}}
...
Furthermore, a restriction is imposed on the pointing angle, {\color{red}{$\theta_{\mathrm{pa}}$}} {\color{blue}{$\theta_{\mathrm{PA}}$}}, between the \Vz momentum and the vector pointing from the primary vertex to the secondary \Vz decay vertex, which is achieved by appointing a minimum value on {\color{red}{$\theta_{\mathrm{pa}}$}} {\color{blue}{$\theta_{\mathrm{PA}}$}} (``Cosine of pointing angle'' in Tables~\ref{tab:LamCuts} and~\ref{tab:K0sCuts}).
...
In order to remove the contamination to the \LamALam and \Ks samples due to misidentification of the protons and pions for each \Vz, the mass assuming different identities (\Lam, \ALam, {\color{blue}{and}} \Ks~{\color{blue}{hypotheses}}) is calculated and utilized in a misidentification procedure.
The {\color{red}{mass assuming}} \Ks hypothesis ($m_{\mathrm{inv,~ K^{0}_{S}~ hyp.}}$) is calculated assuming $\pi^{+}\pi^{-}$ daughters, the {\color{red}{mass assuming}} \Lam hypothesis ($m_{\mathrm{inv,~ \Lambda~ hyp.}}$) assumes p$\pi^{-}$ daughters, and the {\color{red}{mass assuming}} \ALam hypothesis ($m_{\mathrm{inv,~ \overline{\Lambda}~ hyp.}}$) assumes $\overline{\mathrm{p}}\pi^{+}$ daughters. 
...
For \LamALam selection, a candidate is {\color{red}{assumed}} {\color{blue}{concluded}} to be misidentified and is rejected if all of the following criteria are satisfied:
...
\begin{table}[htbp]
 \centering 
 \caption{{\color{red}{\Lam selection criteria}} {\color{blue}{Selection criteria for \Lam and \ALam hyperons}}}
  \renewcommand{\arraystretch}{1.05}
...
\begin{table}[htbp]
 \centering
 \caption{{\color{red}{\Ks selection criteria}} {\color{blue}{Selection criteria for \Ks mesons}}}
  \renewcommand{\arraystretch}{1.05}
  \begin{tabular}{lc|c|l}
   \hlineB{3.0}  
...
\begin{figure}[htp]
  ...
  \subfigure[{\color{red}{p$\pi^{-}$ invariant mass distribution where the \Lam peak is seen.}}]{
    \label{fig:Purity:a}
  ...
  \subfigure[{\color{red}{$\pi^{+}\pi^{-}$ invariant mass distribution where the \Ks peak is seen.}}]{
    \label{fig:Purity:b}
...
In order to reduce the contamination to the two-particle correlations due to pairs sharing daughters and to split or merged tracks, two main pair rejection procedures are applied: a shared daughter restriction, and an average separation constraint.
...
The two-particle correlation function {\color{blue}{for particles $a$ and $b$}}, $C_{ab}(\vec{\mathrm{p}}_{a},\vec{\mathrm{p}}_{b})$, is defined as the ratio of the probability of simultaneously measuring two particles with momenta $p_{a}$ and $p_{b}$, to the product of the single-particle probabilities.
...
The transform matrix describes the decay kinematics of the parent system into the daughter, and is essentially an unnormalized probability distribution mapping the \kstar of the parent pair to that of the daughter pair when one or both parents decay (see {\color{red}{Ref.}}~\cite{Kisiel:2014mma} for more details).
...
\begin{figure}[h]
  ...
  {
  (Color online) THERMINATOR 2 simulation (open {\color{red}{triangles}} {\color{blue}{squares}}) together with experimental data (closed circles).  
...
\begin{table}[htbp]
 \centering 
 \caption[\LamK systematics]{Selection parameter variation for the study of systematic uncertainties in the analysis. In the table, the shorthand used is as follows: {\color{red}{$PA$}} {\color{blue}{PA}}~=~pointing angle; PV~=~primary vertex; DCA~=~distance of closest approach; $\overline{\Delta\mathbf{r}}$~=~average separation}
  \renewcommand{\arraystretch}{1.2}
  \begin{tabular}{l|r}
   \hlineB{3.0} 
   \multicolumn{2}{c}{\LamKs {\color{red}{~systematics}}} \\
   \hlineB{3.0}  
   ...
   \hline
   {\color{red}{$\cos(\theta_{PA})$}} {\color{blue}{$\cos(\theta_{\mathrm{PA}})$}} \LamALam to PV & $>$ [0.9992, 0.9993, 0.9994] \\
   \hline
   {\color{red}{$\cos(\theta_{PA})$}} {\color{blue}{$\cos(\theta_{\mathrm{PA}})$}} \Ks to PV & $>$ [0.9992, 0.9993, 0.9994] \\
   \hline
   ...
   \hlineB{3.0} 
   \multicolumn{2}{c}{\LamKpm {\color{red}{systematics}}} \\
   \hlineB{3.0}  
...
\begin{figure}[h]
  ...
  {\color{red}{[Left:]}} {\color{blue}{(Left) The scattering parameters,}} $\Im f_{0}$ and $\Re f_{0}$, together with $d_{0}$ to the right{\color{blue}{,}} for the \LamKchP (circles), \LamKchM (squares) and \LamKs {\color{red}{~(triangles)}} {\color{blue}{(stars)}} systems.  
  {\color{red}{[Right:] $\lambda$}} {\color{blue}{(Right) The $\lambda_{\mathrm{Fit}}$}} and radius parameters for the 0--10\% (circles), 10--30\% (squares), and 30--50\% {\color{red}{~(triangles)}} {\color{blue}{(stars)}} centrality intervals.  
...
In {\color{red}{Figure}} {\color{blue}{Fig.}}~\ref{fig:ScattParams_3Res} (left), the predictions of {\color{red}{Ref.}}~\cite{Liu:2006xja} do not distinguish the K\Lam and K\ALam interactions and results are shown for two different parameter sets, whereas {\color{red}{Ref.}}~\cite{Mai:2009ce} offers unique K\Lam and $\overline{\mathrm{K}}$\Lam scattering parameters for a single parameter set. 
...
\begin{equation}
\begin{aligned}
 m_{\mathrm{T, pair}}^{2} &= \left( \frac{m_{\mathrm{inv}}}{2} \right)^{2} + \left( \frac{1}{2} |\textbf{\textit{p}}_{\mathrm{T,1}} + \textbf{\textit{p}}_{\mathrm{T,2}}| \right)^{2} = (K^{0})^{2} - (K^{3})^{2}, \quad \mathrm{where} ~~K^{\mu} \equiv \frac{1}{2} \left( p_{1}^{\mu} + p_{2}^{\mu} \right){\color{blue}{.}}
\end{aligned}
\label{eqn:PairmTv1}
\end{equation}
...
The spherical harmonic decomposition of the correlation function offers an elegant method for extracting information about the emission asymmetries{\color{blue}{~\cite{Chajecki:2008vg, PhysRevC.72.054902, Kisiel:2009iw}}}.
...
\subfile{\ResultsDirBaseLamKch\SaveNameModLamKch/Tables/ResultsTableTriple_Vert_CompOldNew.tex}
