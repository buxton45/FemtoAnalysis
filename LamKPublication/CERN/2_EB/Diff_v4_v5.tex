...
\title{\LamK femtoscopy in {\color{red}{Pb-Pb collisions at $\sqrt{s_{\mathrm{NN}}} = $ 2.76 TeV}} {\color{blue}{Pb--Pb collisions at $\mathbf{\sqrt{{\textit s}_{NN}}} =$ 2.76 TeV}}}
\ShortTitle{\LamK femtoscopy in {\color{red}{Pb-Pb}} {\color{blue}{Pb--Pb}} collisions}   % appears on right page headers
...
Current femtoscopic studies are able to extract the size, shape, and orientation of the pair emission regions, as well as {\color{red}{offering}} {\color{blue}{offer}} estimations of the total time to reach kinetic decoupling and the suddenness of particle emission~{\color{blue}{\cite{Lisa:2005dd, Lisa:2008gf}}}.
Non-identical particle analyses additionally allow for the measurement of the {\color{red}{space-time}} {\color{blue}{space--time}} separation of the single particle source emitting regions~{\color{blue}{\cite{Lednicky:1995vk, Voloshin:1997jh, Lednicky:2001qv, Retiere:2003kf}}}.
The momentum and species dependence of femtoscopic measurements affirm the collective nature of the hot and dense matter created in heavy-ion collisions~{\color{blue}{\cite{Makhlin:1987gm, Akkelin:1995gh, Retiere:2003kf, Kisiel:2009eh}}}.
...
Calculations within Quantum Chromodynamics (QCD), the theory of the strong interaction, are notoriously difficult except in {\color{red}{select regimes}} {\color{blue}{the regime}} of weak coupling, where perturbative methods may be applied. 
The \LamK analysis presented offers low energy QCD measurements, which fall into the non-perturbative regime of QCD.
Therefore, the \LamK {\color{red}{~measurements}} {\color{blue}{analysis}} not only give{\color{blue}{s}} insight into the strong interaction, {\color{red}{they}} {\color{blue}{it}} will also help guide future QCD calculations.
This study is particularly interesting, as the \LamK scattering parameters {\color{red}{are not}} {\color{blue}{were not previously}} known, and theoretical predictions are limited.
The extracted scattering parameters are compared to predictions obtained in the framework of chiral perturbation theory~\cite{Liu:2006xja,Mai:2009ce}{\color{red}{; neither predict a repulsive interaction, as observed in the \LamKchP system}}.
...
The scattering parameters, along with pair emission source sizes, are extracted with a femtoscopic analysis of \LamK correlations in {\color{red}{Pb-Pb}} {\color{blue}{Pb--Pb}} collisions at $\sqrt{s_{\mathrm{NN}}}$ = 2.76 TeV {\color{red}{from}} {\color{blue}{measured by}} the ALICE experiment at the LHC.  
...
Throughout the text, the pair name is used as shorthand for the pair-conjugate system, which are found to be consistent {\color{red}{(e.g. \LamKs, \LamKchP $\oplus$ \ALamKchM is simply \LamKchP)}} {\color{blue}{(e.g., \LamKchP for \LamKchP $\oplus$ \ALamKchM, \LamKchM for \LamKchM $\oplus$ \ALamKchP, and \LamKs for \LamKs $\oplus$ \ALamKs), and \LamK is used to describe all \LamK combinations}}.
...
{\color{red}{\Kpm track detection utilized both TPC and TOF detectors}} {\color{blue}{Track reconstruction for the charged kaons was performed using the TPC}}, and tracks within the range 0.14 $<$ \pt $<$ 1.5 GeV/$c$ were accepted.
To reduce the number of {\color{red}{secondaries}} {\color{blue}{secondary particles}} (e.g.{\color{blue}{,}} charged particles produced in the detector material, particles from weak decays, etc.) in the sample, a {\color{red}{maximum cut is established on the}} {\color{blue}{selection criterion is established on the maximum}} distance-of-closest-approach (DCA) of the track to the primary vertex.
This {\color{red}{restriction}} is realized by imposing a {\color{red}{DCA cut}} {\color{blue}{restriction on the DCA}} in both the transverse and beam directions.

{\color{red}{PID}} {\color{blue}{Particle identification}} was performed using both the TPC and TOF detectors via the $N_{\sigma}$ method. 
The $N_{\sigma}$ {\color{red}{cuts}} {\color{blue}{selection criteria}} become tighter with increasing momentum to reduce contamination within the samples, as the \Kpm signals begin to overlap more significantly with those from other particles, particularly e$^{\pm}$ and $\pi^{\pm}$.
Additional methods are included to reduce the contamination in the \Kpm samples from the electrons and pions.  
The specifics for {\color{red}{these cuts}} {\color{blue}{the \Kpm selection}} are contained in Table~\ref{tab:KchCuts}.
...
For a more detailed estimate of the \Kpm purity from an analysis employing similar {\color{red}{cuts}} {\color{blue}{methods}}, see Ref.~\cite{Acharya:2017qtq}.


\begin{table}[htbp]
 \centering
 \caption{{\color{blue}{Charged kaon (\Kpm) selection criteria}}}
  ...
  \begin{tabular}{lcc|c|l}
   ...
   \multicolumn{5}{l}{TPC and TOF $N_{\sigma}$ {\color{red}{Cuts}}} \\
   \cline{2-5}
   ...
   \multicolumn{4}{l|}{\multirow{3}{*}{Electron {\color{red}{R}}{\color{blue}{r}}ejection: {\color{red}{R}}{\color{blue}{r}}eject if all satisfied}} & $N_{\sigma e^{-},\mathrm{TPC}} < $ 3 \\
...
Aside from typical kinematic and PID {\color{red}{cuts}} {\color{blue}{selection methods}} (using TPC and TOF detectors), the {\color{red}{daughter tracks}} {\color{blue}{tracks of the decay products (called \textit{daughters})}} are also exposed to a minimum {\color{red}{cut on}} {\color{blue}{requirement of}} their impact parameter with respect to the primary vertex.  
...
To select primary candidates, {\color{red}{each \Vz is exposed to a maximum cut on its impact parameter with respect to the primary vertex}} {\color{blue}{the impact parameter with respect to the primary vertex is used as a selection criterion for each \Vz}}.
Furthermore, a {\color{red}{selection}} {\color{blue}{restriction}} is imposed on the pointing angle, $\theta_{\mathrm{pa}}$, between the \Vz momentum and the vector pointing from the primary vertex to the secondary \Vz decay vertex, which is achieved by appointing a minimum value on $\cos(\theta_{\mathrm{pa}})$ (``Cosine of pointing angle'' in Tables~\ref{tab:LamCuts} and~\ref{tab:K0sCuts}).
...
\begin{table}[htbp]
 \centering
 \caption{{\color{blue}{\Ks selection criteria}}}
  \renewcommand{\arraystretch}{1.05}
  \begin{tabular}{lc|c|l}
   \hline  
   ...
   \multicolumn{3}{l|}{Cosine of pointing angle} & $>$ 0.9993 \\
   \hline
   \multicolumn{3}{l|}{Decay {\color{red}{L}}{\color{blue}{l}}ength} & $<$ 30 cm \\
   \hline
      
   
   \multicolumn{4}{c}{\textbf{$\pi^{\pm}$ {\color{red}{D}}{\color{blue}{d}}aughter {\color{red}{Cuts}} {\color{blue}{criteria}}}} \\
   \hline
   \multicolumn{3}{l|}{$p_{\mathrm{T}}$} & $>$ 0.15 GeV/\textit{c} \\
   \hline
   \multicolumn{3}{l|}{$|\eta|$} &  $< 0.8$ \\
   \hline
   \multicolumn{3}{l|}{DCA $\pi^{+}\pi^{-}$ {\color{red}{D}}{\color{blue}{d}}aughters} & $<$ 0.3 cm \\
   \hline
   \multicolumn{3}{l|}{DCA to primary vertex} & $>$ 0.3 cm \\
   \hline
   \multicolumn{4}{l}{TPC and TOF $N_{\sigma}$ {\color{red}{Cuts}}} \\
  ...
  \end{tabular}
% \end{minipage}
 \caption{{\color{red}{\Ks selection}}}
 \label{tab:K0sCuts} 
\end{table}
...
\begin{figure}[htp]
  ...
  %%----overall caption----
  \caption{
  (Color online) Invariant mass (\minv) distribution of {\color{blue}{(a)}} p$\pi^{+}$ pairs showing the \Lam peak {\color{red}{\ref{fig:Purity:a}}}, and of {\color{blue}{(b)}} $\pi^{+}\pi^{-}$ pairs showing the \Ks peak {\color{red}{\ref{fig:Purity:b}}}, for \Vz candidates.  
  The bottom panels are zoomed to show the background with fit.  
  The vertical dashed lines represent the \minv {\color{red}{~cuts}} {\color{blue}{restrictions}} used in the analyses, the vertical dotted lines delineate the region over which the background was fit, and the dash-dotted line shows the background fit.
  }  
  \label{fig:Purity}
\end{figure}
...
%************************************************************************************************************************
\subsection{Pair {\color{red}{C}}{\color{blue}{c}}onstruction}
\label{PairConstruction}

In order to reduce the contamination to the two-particle correlations due to pairs sharing daughters and to split or merged tracks, two main {\color{red}{pair cuts}} {\color{blue}{rejection procedures}} are applied: a shared daughter {\color{red}{cut}} {\color{blue}{restriction}}, and an average separation {\color{red}{cut}} {\color{blue}{constraint}}.
The purpose of the shared daughter {\color{red}{cut}} {\color{blue}{restriction}} is to ensure the first particle in the pair is unique from the second.  
For pairs formed of two V$^{0}$s (i.e.{\color{blue}{,}} \LamKs), this {\color{red}{cut}} is implemented by removing all pairs which share a daughter.  
For a pair formed of a single \Vz and a charged track (i.e.{\color{blue}{,}} \LamKpm), the {\color{red}{cut}} {\color{blue}{restriction}} removes all pairs in which the charged track is also claimed as a daughter of the \Vz.  
...
\begin{equation}
  C_{ab}(\vec{\mathrm{p}}_{a},\vec{\mathrm{p}}_{b}) = \frac{E_{a}E_{b}\frac{dN_{ab}}{d^{3}p_{a}d^{3}p_{b}}}{\big( E_{a}\frac{dN_{a}}{d^{3}p_{a}} \big) \big( E_{b}\frac{dN_{b}}{d^{3}p_{b}} \big)}{\color{blue}{,}}
\label{eqn:CfRatioSpectra}
\end{equation}
{\color{blue}{where $N_{ab}$ is the yield of particle pairs, $E_{a(b)}$ is the energy, $p_{a(b)}$ is the three-momentum, and $N_{a(b)}$ is the yield of particles $a(b)$.}}
{\color{red}{This may be expressed theoretically as in the Koonin-Pratt equation \cite{Koonin:1977fh, Pratt:1990zq}:}}
{\color{blue}{Theoretically, the correlation function may be expressed as in the Koonin-Pratt equation~\cite{Koonin:1977fh, Pratt:1990zq},}}
\begin{equation}
 C(\mathbf{k^{*}}) = \int S_{\mathbf{P}}(\mathbf{r^{*}})|\Psi_{\mathbf{k^{*}}}(\mathbf{r^{*}})|^{2}d^{3}\mathbf{r^{*}}{\color{blue}{,}}
...
In practice, the correlation function is formed experimentally as{\color{red}{:}}
\begin{equation}
  C(k^{*}) = \mathcal{N}\frac{A(k^{*})}{B(k^{*})}{\color{blue}{,}}
\label{eqn:CfExp}
\end{equation}
where $A(k^{*})$ is the signal distribution, $B(k^{*})$ is the reference distribution, and $\mathcal{N}$ is a normalization parameter.  
{\color{red}{$B(k^{*})$}} {\color{blue}{The reference distribution}} is used to divide out the phase-space effects, leaving only the femtoscopic effects in the correlation function. 
...
{\color{red}{$A(k^{*})$ is constructed by binning in \kstar pairs from the same event.}}
{\color{blue}{The signal distribution is the same-event distribution of particle pairs.}}
Typically, $B(k^{*})$ is obtained using mixed-event pairs~\cite{Kopylov:1974th}, i.e.{\color{blue}{,}} particles from a given event are paired with those from another event.
...
{\color{red}{In order to mix only similar events, events are binned both in primary vertex location (2 cm bin width) and in centrality (5\% bin width), and only events within a given bin are mixed; i.e. only events of like centrality and of like primary vertex location are mixed.}}
{\color{blue}{In order to mix similar events, only events of like centrality (within 5\%) and of like primary vertex position (within 2 cm) are mixed.}}

This analysis presents correlation functions for three centrality {\color{red}{bins}} {\color{blue}{percentile ranges}} (0--10\%, 10--30\%, and 30--50\%), and is pair transverse momentum ($k_{\mathrm{T}} = \frac{1}{2}|\mathbf{p}_{\mathrm{T,1}}+\mathbf{p}_{\mathrm{T,2}}|$) integrated ({\color{red}{i.e. not binned in}} {\color{blue}{i.e., no restriction on}} $k_{\mathrm{T}}$) due to limited data.
The $k_{\mathrm{T}}$ dependences of the three \LamK charge combinations {\color{red}{are}} {\color{blue}{should be}} comparable, so an integrated analysis is acceptable.
...
\begin{table}[htbp]
 \centering
 \caption{{\color{blue}{Weight parameters ($\lambda_{ij}$) for the individual components of the \LamK correlation functions}}}
...
A significant non-femtoscopic background is observed in all of the studied \LamK correlations{\color{blue}{,}} which increases with decreasing centrality, is the same amongst all \LamKpm pairs, and is more pronounced in the \LamKs system (the difference in \LamKpm and \LamKs backgrounds is due mainly to a difference in kinematic {\color{red}{cuts}} {\color{blue}{selection criteria}}).  
The background is {\color{red}{due primarily}} {\color{blue}{primarily due}} to particle collimation associated with elliptic flow, and results from mixing events with unlike {\color{red}{event-plane angles}} {\color{blue}{event planes}}\footnote[1]
{\color{red}{
{
...
The effect produces the observed suppression at {\color{red}{intermediate-\kstar}} {\color{blue}{intermediate \kstar}}, and should also lead to an enhancement at {\color{red}{low-\kstar}} {\color{blue}{low \kstar}}.
The behavior of the non-femtoscopic background is needed in the {\color{red}{low-\kstar}} {\color{blue}{low \kstar}} femtoscopic signal region, but {\color{red}{a clean}} {\color{blue}{an isolated}} view of it is only possible outside of such a region.
...
Figure~\ref{fig:BgdswTHERM} shows the THERMINATOR 2 simulation {\color{red}{(open triangles)}} together with experimental data {\color{red}{(closed circles)}}.  
The figure also shows a 6$^{\mathrm{th}}$-order polynomial fit to the simulation {\color{red}{(dashed curves)}}, as well as the fit polynomial scaled to match the data {\color{red}{(solid curves)}}.
...
Finally, each correlation function has a unique normalization parameter{\color{blue}{, $\mathcal{N}$}}.
...
\begin{table}[htbp]
 \centering 
 \caption[\LamK systematics]{{\color{blue}{Selection parameter variation for the study of systematic uncertainties in the analysis. In the table, the shorthand used is as follows: $PA$ = pointing angle; PV = primary vertex; DCA = distance of closest approach; $\overline{\Delta\mathbf{r}}$ = average separation}}}
  \renewcommand{\arraystretch}{1.2}
  \begin{tabular}{l|r}
   \hlineB{3.0} 
  ...
  \end{tabular}
% \end{minipage}
 \caption[\LamK systematics]{{\color{red}{\LamK systematics. In the table, the shorthand used is as follows: $PA$ = pointing angle; PV = primary vertex; DCA = distance of closest approach; $\overline{\Delta\mathbf{r}}$ = average separation}}}
...
Figure~\ref{fig:LamKFits_3Res} shows the \LamK data with fits for all studied centrality {\color{red}{bins}} {\color{blue}{percentile intervals}} (0--10\%, 10--30\%, and 30--50\%). 
All six \LamK systems (\LamKchP, \ALamKchM, \LamKchM, \ALamKchP, \LamKs, \ALamKs) are fit simultaneously across all centralities, with a single radius and normalization $\lambda_{\mathrm{Fit}}$ parameter for each centrality {\color{red}{bin}} {\color{blue}{interval}}.
Scattering parameters ($\Re f_{0}$, $\Im f_{0}$, $d_{0}$) are shared between pair-conjugate systems, but assumed unique {\color{red}{between}} {\color{blue}{among}} the different \LamK charge combinations (i.e., a parameter set describing the \LamKchP \& \ALamKchM system, a second set describing the \LamKchM \& \ALamKchP system, and a third for the \LamKs \& \ALamKs system).
...
The figure demonstrates that the final fit function is similar to the primary \LamK component, with the largest differences between the two observed in the {\color{red}{30-50\%}} {\color{blue}{30--50\%}} centrality {\color{red}{bin}} {\color{blue}{interval}} due mainly to the large contribution of the non-femtoscopic background.
...
\begin{figure}[h]
  ...
  {\color{red}{The cross ([A] = Ref.\ \cite{Liu:2006xja}) and X symbols ([B] = Ref.\ \cite{Mai:2009ce}) points}} {\color{blue}{The cross~\cite{Liu:2006xja} and X~\cite{Mai:2009ce} points}} show theoretical predictions made using chiral perturbation theory.
  }
...
The $\Re f_{0}$ extracted for the \LamKs system is positive, and within {\color{red}{errors}} {\color{blue}{uncertainties}} of that of the \LamKchM. 
...
As is the usual convention in femtoscopy, a positive $\Re(f_{0})$ {\color{red}{(\LamKchM, \LamKs)}} signifies that the effect of the interaction is attractive, while a negative $\Re(f_{0})$ {\color{red}{(\LamKchP)}} signifies a repulsive interaction.
{\color{red}{Past studies of kaon-proton scattering found the K$^{-}$--p interaction to be attractive, and that of the K$^{+}$--p to be repulsive \cite{Humphrey:1962zz, Hadjimichef:2002xe, Ikeda:2012au}.}}
{\color{blue}{Therefore, the femtoscopic signals from this analysis demonstrate that the strong interaction acts repulsively in the \LamKchP system, and acts attractively in the \LamKchM and \LamKs systems.}}
...
This difference could be due to an effect arising from different {\color{red}{quark-antiquark}} {\color{blue}{quark--antiquark}} interactions between the pairs ($\rm s\overline{s}$ in \LamKchP, $\rm u\overline{u}$ in \LamKchM).
...
Figure~\ref{fig:ScattParams_3Res} (right) presents the $\lambda_{\mathrm{Fit}}$ {\color{red}{vs.}} {\color{blue}{and}} radius parameters for all three studied centrality {\color{red}{bins}} {\color{blue}{percentile ranges}}.
...
The figure shows extracted $R_{\mathrm{inv}}$ {\color{red}{vs.}} {\color{blue}{as a function of}} \mt for several {\color{red}{centralities}} {\color{blue}{centrality ranges}} and for several different pair systems.
...
These kinematic effects, resulting from the transformation from LCMS to PRF, cause{\color{red}{s}} smaller masses to exhibit larger $R_{\mathrm{inv}}$~\cite{Adam:2015vja} (explaining, for instance, how the pion radii are systematically higher than kaon radii at the same approximate \mt).
...
\begin{figure}[h!]
  ...
  {
  {\color{red}{(Color online) $C_{00}$ (left) and $\Re C_{11}$ (right) components of a spherical harmonic decomposition of the \LamKchP correlation function for the 0--10\% centrality bin.}}
  {\color{blue}{(Color online) Spherical harmonics components $C_{00}$ (left) and $\Re C_{11}$ (right) of the \LamKchP correlation function for the 0--10\% centrality interval.}}
The $C_{00}$ component is similar to the {\color{red}{1D}} {\color{blue}{one-dimensional}} correlation functions typically studied, and probes the overall size of the source.
...
With the Stavinskiy method, mixed-event pairs are not used for the reference distribution; instead, same-event pseudo-pairs, formed by rotating one particle in a real pair by 180$^\circ$ in the transverse plane, are used.  
This rotation rids the pairs of any femtoscopic correlation, while maintaining correlations due to elliptic flow (and other suitably symmetric contributors).
Care needs to be taken in treating the pseudo-pairs exactly like the real pairs; e.g.{\color{blue}{,}} the pseudo-pairs should be exposed to the same pair {\color{red}{cuts}} {\color{blue}{rejection procedures}} used in the analysis on the real pairs.
The results of correctly implementing such a procedure are shown in Fig.~\ref{fig:StavCfs_Correct_LamKchP}.
{\color{red}{The figure shows the Stavinskiy method does a very good job of ridding the \LamKchP correlations of their non-femtoscopic backgrounds.}}
{\color{blue}{The figure demonstrates, for the \LamKchP system, that the Stavinskiy method is effective in flattening the correlation function in the region where no femtoscopic signal is expected.}}
...
The top row of Fig.~\ref{fig:LamKchP_StdThermSources} shows the experimental $\Lambda\mathrm{K}^{+}\oplus\overline{\Lambda}\mathrm{K}^{-}$ data together with the simulation results {\color{blue}{(a)}} for the one-dimensional correlation function {\color{red}{(top left)}} and {\color{blue}{(b)}} for {\color{red}{for the $\Re C_{11}$ component}} {\color{blue}{the real part of the $l=1$, $m=1$ component, $\Re C_{11}$,}} of the spherical harmonic decomposition {\color{red}{(top right)}}.
The other four plots in Fig.~\ref{fig:LamKchP_StdThermSources} show {\color{red}{the source distribution from the simulation in the out (middle left), side (middle right), and long (bottom left) directions, as well as the temporal characteristics of the source (bottom right)}} {\color{blue}{the two-particle emission function (i.e., the pair separation distributions) from the simulation in the (c) out ($r^{*}_{\mathrm{out}}$), (d) side ($r^{*}_{\mathrm{side}}$), and (e) long ($r^{*}_{\mathrm{long}}$) directions, as well as (f) the temporal characteristics of the source ($\Delta t^{*}$)}}, all measured in the PRF.
...
Figure~\ref{fig:LamKchP_ThermSources_VaryMuOut_SH} shows, together with the experimental \LamKchP data, the effect of increasing $\mu_{\mathrm{out}}$ on the {\color{red}{$\Re C_{00}$ and $\Re C_{11}$ components of the spherical harmonic decomposition}} {\color{blue}{spherical harmonic $l=0$, $m=0$ component, $C_{00}$, and on the real part of the $l=1$, $m=1$ component, $\Re C_{11}$}}.
...
\begin{figure}[h]
  ...
  {
  (Color online) {\color{red}{$C_{00}$ (left) and $\Re C_{11}$ (right) components of a spherical harmonic decomposition}} {\color{blue}{Spherical harmonics components (left) $C_{00}$ and (right) $\Re C_{11}$}} of the \LamKchP correlation function for the 0--10\% centrality {\color{red}{bin}} {\color{blue}{interval}} shown with results from the THERMINATOR 2 simulation implemented with different shifts in the outward direction, $\mu_{\mathrm{out}}$, as described in the text.
  }
