...
Examples of such distribtions can be found in Figure \ref{fig:Purity}].
...
As shown in Figure \ref{fig:Purity}, the background is fit (with a polynomial) outside of the peak region of interest to obtain an estimate for the backfround within the region.
...
\begin{itemize}
 ...
 \item Positive and negative daughters pass $\pi$ daughter cut implemented for \Ks reconstruction
...
Figure \ref{fig:Purity:a} shows the invariant mass (m$_{\mathrm{inv}}$) distribution of all \Lam candidates immediately before the final invariant mass cut.
This distribution (and similar for \ALam) is used to calculate the collection purity.
The \Lam and \ALam purities are found to be: Purity(\Lam) $\approx$ Purity(\ALam) $\approx$ 95\%.


\subsubsection{K$^{0}_{\mathrm{S}}$ selection}
...
This is how we select \Kpm candidates.
...
Stuff can also be seen in \ref{fig:KchPdEdx}
...
\begin{enumerate}
 ...
 \begin{enumerate}
  \item V0-V0 Pairs (i.e. \LamALamKs analyses)
  \begin{itemize}
   ...
   \end{itemize} 
  \end{itemize}
  \item V0-Track Pairs (i.e. \LamALamKpm analyses)
  \begin{itemize}
   \item Remove pairs if Track is also used as a daughter of the V0
   \begin{itemize}
    \item In these analyses, this could only occur if, for instance, a \Kpm is misidentified as a $\pi$ or $p$ in the V0 reconstruction
   \end{itemize}
  \end{itemize}  
 \end{enumerate}
...
Figures \ref{fig:cLamK0Cfs}, \ref{fig:LamKchPwConjCfs}, and \ref{fig:LamKchMwConjCfs} show the correlation functions for all centalities studied for \LamKsALamKs, \LamKchPALamKchM, and \LamKchMALamKchP, respectively. All were normalized in the range 0.32 $< k^{*} < $ 0.4 GeV/$c$.
