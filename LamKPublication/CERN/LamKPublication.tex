...
Additional methods are included to reduce the contamination in the \Kpm samples from electrons and pions.  
...
\begin{figure}[htp]
  ...
  %%----overall caption----
  \caption{
  (Color online) Invariant mass (\minv) distribution in the 0--10\% centrality interval of (a) p$\pi^{+}$ pairs showing the \Lam peak, and of (b) $\pi^{+}\pi^{-}$ pairs showing the \Ks peak, for \Vz candidates.  
...
The \XiKpm correlation function is dominated by the contribution from the Coulomb interaction, and consistent final fit results are obtained when modeling the \XiKpm system with a Coulomb-only scenario, in which the strong interaction is assumed to be negligible, instead of using the experimental data.
...
Note that, in each case, the central value corresponds to that used in the analysis.
