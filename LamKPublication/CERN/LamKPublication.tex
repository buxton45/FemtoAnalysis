...
Current femtoscopic studies are able to extract the size, shape, and orientation of the pair emission regions, as well as offer estimations of the total time to reach kinetic decoupling and the suddenness of particle emission.
...
The dataset analyzed is from Pb--Pb collisions at $\sqrt{s_{\mathrm{NN}}}$ = 2.76 TeV at the LHC measured by the ALICE detector \cite{Aamodt:2008zz} in 2011.
...
The reconstruction efficiencies ($RE_{ij}$) are estimated with HIJING simulations, which have been run through GEANT to simulate the detector response.
...
For this hodgepodge of pair systems, all with different two-particle interactions and single-particle source distributions, the net correlation effect is assumed to average to unity.
...
The background is primarily due to particle collimation associated with elliptic flow, and results from mixing events with unlike event-plane angles\footnote[1]
{
An attempt was made to decrease the background by additionally grouping events in $\Psi_{\mathrm{EP}}$, but only a small reduction in the signal was achieved due to the limited event-plane resolution.
}\cite{Kisiel:2017}.
...
\begin{figure}[h]
  ...
  [Right]: $\lambda$ vs. Radius for the 0--10\% (circles), 10--30\% (squares), and 30--50\% (triangles) centrality intervals.  
...
Figure \ref{fig:ScattParams_3Res} (right) presents the $\lambda_{\mathrm{Fit}}$ vs. radius parameters for all three studied centrality percentile ranges.
...
The figure shows extracted $R_{\mathrm{inv}}$ vs. \mt for several centrality ranges and for several different pair systems.
...
\begin{figure}[h!]
  ...
  {
  (Color online) $C_{00}$ (left) and $\Re C_{11}$ (right) components of a spherical harmonic decomposition of the \LamKchP correlation function for the 0--10\% centrality interval.  
...
$\delta_{\mathrm{c}}$ is the Coulomb s-wave phase shift, $A_{\mathrm{c}}(\eta)$ is the Coulomb penetration factor, $\tilde{G} = \sqrt{A_{c}}(G_{0} + iF_{0})$ is a combination of the regular ($F_{0}$) and singular ($G_{0}$) s-wave Coulomb functions.  
$f_{\mathrm{c}}(k^{*})$ is the s-wave scattering amplitude,
\begin{equation}
