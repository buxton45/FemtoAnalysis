...
{\color{blue}{Appendix \ref{App:CoulombFitter} discusses the procedure needed to generate fit functions when both the strong and Coulomb interactions are present.}}
...
{\color{red}{Finally, to avoid any auto-correlation effects, all \Vz candidates are ensured to have unique daughters.
...
{\color{blue}{To avoid any auto-correlation effects, all \Vz candidates within each single-particle collection (\Lam, \ALam, and \Ks separately) are ensured to have unique daughters. 
...
The $\lambda_{ij}$ parameters serve as weights dictating the relative strength of each component's contribution to the observed signal, and are normalized to unity (i.e. $\sum_{ij} \lambda_{ij} = 1$, where $ij$ includes also the primary \LamK component) {\color{blue}{\cite{Kisiel:2014mma, Acharya:2018gyz}}}.
...
{\color{red}{In practice, the}} {\color{blue}{The}} contribution of a parent system (e.g. $\Sigma^{0}$\KchP) to the daughter correlation function (e.g. \LamKchP) is determined by modeling the parent system's correlation function and running it through the appropriate transform matrix.
{\color{red}{Since the interactions between these particles are not known, all residual pairs are assumed to have the same source size as the daughter pair.
...
{\color{blue}{Since the interactions between these particles are not known, some assumptions must be made.
...
More specifically, a $\lambda_{ij}$ parameter is estimated as the total number of \LamK pairs in the experimental sample originating from $ij$ ($N_{ij}$) divided by the total number of \LamK pairs {\color{red}{($N_{\mathrm{Total}}$)}}.
...
{\color{red}{The description by THERMINATOR 2 of the non-femtoscopic backgrounds in the \LamKpm systems is remarkable, and can be used in a quantitative fashion to help fit the data.}}
{\color{blue}{The THERMINATOR 2 simulation offers a good description of the non-femtoscopic backgrounds in the \LamK systems, and can be used in a quantitative fashion to help fit the data.}}
{\color{red}{More specifically, the non-femtoscopic backgrounds were modeled by (6$^{\mathrm{th}}$-order) polynomial fits to THERMINATOR 2 simulation for the \LamKpm analyses; one polynomial for each centrality class.}}
{\color{blue}{More specifically, the non-femtoscopic backgrounds are modeled by 6$^{\mathrm{th}}$-order polynomial fits to THERMINATOR 2 simulation
\begin{equation}
...
The {\color{red}{form}} {\color{blue}{coefficients}} of each polynomial are set before use with the experimental data by fitting to the THERMINATOR 2 simulation, shown in Fig.\ \ref{fig:BgdswTHERM}.
The extracted polynomial is adjusted to best describe the experimental data by introducing a scale factor and a vertical shift {\color{red}{both of which}},
{\color{blue}{\begin{equation}
F_{\scaleto{Bgd}{6pt}}(k^{*}) = \alpha\cdot F_{\scaleto{THERM.\; Bgd}{6pt}}(k^{*}) + \beta
\end{equation}
where $\alpha$ and $\beta$}} are determined by fitting to {\color{blue}{the}} data in the region $0.32 < k^{*} < 0.80$ GeV/$c$; during the fit of the low-\kstar signal region, the background is fixed.
...
\begin{figure}[h]
  ...
  {
  (Color online) Extracted fit $R_{\mathrm{inv}}$ parameters as a function of pair transverse mass (\mt) for {\color{red}{various pair systems over}} several centralities.
  {\color{red}{The ALICE published data \cite{Adam:2015vja} is shown with transparent, open symbols.}}
  {\color{blue}{Results from the \LamK analysis are presented together with ALICE published data \cite{Adam:2015vja} for various other pair systems.}}
  }
  \label{fig:mTScalingOfRadii_3Res}
\end{figure}

Figure \ref{fig:ScattParams_3Res} (right) presents the {\color{red}{$\lambda$}} {\color{blue}{$\lambda_{\mathrm{Fit}}$}} vs. radius parameters for all three studied centrality bins.
{\color{blue}{The $\lambda_{\mathrm{Fit}}$ parameters are expected to be close to unity.}}
{\color{red}{Furthermore, a}} {\color{blue}{A}} comparison of the extracted radii from this study to those of other systems measured by ALICE \cite{Adam:2015vja} is shown in Figure \ref{fig:mTScalingOfRadii_3Res}. 
...
{\color{red}{They also demonstrate a decreasing size with increasing \mt in identical particle studies, as expected in the presence of collective radial flow \cite{Akkelin:1995gh}.}}
{\color{blue}{For each pair system, the radii decrease with increasing \mt, as expected in the presence of collective radial flow \cite{Akkelin:1995gh}.}}
It was found that \cite{Kisiel:2014upa}, even in the presence of good global \mt-scaling for the three-dimensional radii in the Longitudinally Co-Moving System (LCMS), a particle species dependence will exist for the $R_{\mathrm{inv}}$ measured in the {\color{red}{Pair Rest Frame (PRF)}} {\color{blue}{PRF}}, due to trivial kinematic reasons.
...
{\color{blue}{The use of this method in a three-dimensional analysis of two-pion correlations produced, in comparison to the event mixing results, an increase of 6\% for $R_{\mathrm{side}}$ at low-$k_{\mathrm{T}}$ and up to 4\% for $R_{\mathrm{out}}$ and $R_{\mathrm{long}}$ \cite{Aamodt:2011mr}.}}
