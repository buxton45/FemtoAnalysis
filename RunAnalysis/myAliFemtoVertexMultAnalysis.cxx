...
#ifdef __ROOT__ 
ClassImp(myAliFemtoVertexMultAnalysis)
#endif
...
myAliFemtoVertexMultAnalysis::myAliFemtoVertexMultAnalysis(char* name, unsigned int binsVertex, double minVertex, double maxVertex,
...
myAliFemtoVertexMultAnalysis::myAliFemtoVertexMultAnalysis(const myAliFemtoVertexMultAnalysis& a) : 
  ...
  fOutputName(a.fOutputName)    
{
  ...
  if ( fEventCut ) {
      ...
      cout << " myAliFemtoVertexMultAnalysis::myAliFemtoVertexMultAnalysis(const myAliFemtoVertexMultAnalysis& a) - event cut set " << endl;
  }
  if ( fFirstParticleCut ) {
      SetFirstParticleCut(fFirstParticleCut); // this will set the myAnalysis pointer inside the cut
      cout << " myAliFemtoVertexMultAnalysis::myAliFemtoVertexMultAnalysis(const myAliFemtoVertexMultAnalysis& a) - first particle cut set " << endl;
  }
  if ( fSecondParticleCut ) {
      SetSecondParticleCut(fSecondParticleCut); // this will set the myAnalysis pointer inside the cut
      cout << " myAliFemtoVertexMultAnalysis::myAliFemtoVertexMultAnalysis(const myAliFemtoVertexMultAnalysis& a) - second particle cut set " << endl;
  }  if ( fPairCut ) {
      SetPairCut(fPairCut); // this will set the myAnalysis pointer inside the cut
      cout << " myAliFemtoVertexMultAnalysis::myAliFemtoVertexMultAnalysis(const myAliFemtoVertexMultAnalysis& a) - pair cut set " << endl;
  }

  AliFemtoCorrFctnIterator iter;
  for (iter=a.fCorrFctnCollection->begin(); iter!=a.fCorrFctnCollection->end();iter++){
    cout << " myAliFemtoVertexMultAnalysis::myAliFemtoVertexMultAnalysis(const myAliFemtoVertexMultAnalysis& a) - looking for correlation functions " << endl;
    AliFemtoCorrFctn* fctn = (*iter)->Clone();
    if (fctn) AddCorrFctn(fctn);
    else cout << " myAliFemtoVertexMultAnalysis::myAliFemtoVertexMultAnalysis(const myAliFemtoVertexMultAnalysis& a) - correlation function not found " << endl;
  }

  fNumEventsToMix = a.fNumEventsToMix;

  cout << " myAliFemtoVertexMultAnalysis::myAliFemtoVertexMultAnalysis(const myAliFemtoVertexMultAnalysis& a) - analysis copied " << endl;

}
myAliFemtoVertexMultAnalysis& myAliFemtoVertexMultAnalysis::operator=(const myAliFemtoVertexMultAnalysis& TheOriginalAnalysis)
{
  if (this != &TheOriginalAnalysis) {
    myAliFemtoVertexMultAnalysis::operator=(TheOriginalAnalysis);
    fVertexZBins = TheOriginalAnalysis.fVertexZBins; 
    fMultBins = TheOriginalAnalysis.fMultBins;
    fOutputName = TheOriginalAnalysis.fOutputName;
...
//____________________________
myAliFemtoVertexMultAnalysis::~myAliFemtoVertexMultAnalysis(){
  // now delete every PicoEvent in the EventMixingBuffer and then the Buffer itself
  delete fPicoEventCollectionVectorHideAway;
}

//____________________________
AliFemtoString myAliFemtoVertexMultAnalysis::Report()
{
  // Prepare a report of the execution
  cout << "myAliFemtoVertexMultAnalysis - constructing Report..."<<endl;
  char ctemp[200];
  AliFemtoString temp = "-----------\nHbt myAliFemtoVertexMultAnalysis Report:\n";
...
void myAliFemtoVertexMultAnalysis::ProcessEvent(const AliFemtoEvent* hbtEvent) {
  // Perform event processing
  // in bins of z vertex and multiplicity

  // cout << " myAliFemtoVertexMultAnalysis::ProcessEvent(const AliFemtoEvent* hbtEvent) " << endl;
