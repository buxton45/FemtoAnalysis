...
AliFemtoManager* ConfigFemtoAnalysis() 
{
  ...
  //Setup the analysis
  myAliFemtoVertexMultAnalysis *analy = new myAliFemtoVertexMultAnalysis("analy",8, -8.0, 8.0, 4, 0, 100);
    ...
    analy->SetNumEventsToMix(5);
    analy->SetMinSizePartCollection(1);
    analy->SetVerboseMode(kFALSE);
...
//  v0pc1->SetShareFractionMax(0.05)  //how do I implement this in AliFemtoV0PairCut?
  ...
  analy->SetEventCut(mec);
  analy->SetFirstParticleCut(k0cut1);
  analy->SetSecondParticleCut(v0cut2);
  analy->SetPairCut(v0pc1);
//  analy->SetPairCut(dumbpaircut);


  // Setup correlation functions
  AliFemtoQinvCorrFctn *cqinv= new AliFemtoQinvCorrFctn("qinvcf",75,0.0,0.4);
  myAliFemtoKStarCorrFctn *mycKStar = new myAliFemtoKStarCorrFctn("myKStarcf",75,0.0,0.4);
  KStarCF *KubcKStar = new KStarCF("KubKStarcf",75,0.0,0.4);

  // add the correlation functions to the analysis
  analy->AddCorrFctn(cqinv);
  analy->AddCorrFctn(mycKStar);
  analy->AddCorrFctn(KubcKStar);

  // Add the analysis to the manager
  mgr->AddAnalysis(analy);
