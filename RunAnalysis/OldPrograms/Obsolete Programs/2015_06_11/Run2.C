...
int *use_runs;
use_runs = runs_subset;

bool rungrid = true;
TString aConfigMacro = "ConfigFemtoAnalysis_K0Lam2.C";
TString AdditionalLibs = "libPWGCFfemtoscopy.so libPWGCFfemtoscopyUser.so myAliFemtoV0TrackCut.h myAliFemtoV0TrackCut.cxx myAliFemtoKStarCorrFctn.h myAliFemtoKStarCorrFctn.cxx KStarCF.h KStarCF.cxx myAliFemtoVertexMultAnalysis.h myAliFemtoVertexMultAnalysis.cxx ConfigFemtoAnalysis_K0Lam2.C";
TString aGridWorkingDir = "cLamK0_ss_OLD";
TString aGridOutputDir = "output_data";
TString outputname = "Resultsgrid_cLamK0_ss_OLD.root";
...
void Run2()
{
    ...
    // Setup includes
    gInterpreter->AddIncludePath("$ALICE_BUILD/include");
    gInterpreter->AddIncludePath("$ALICE_ROOT/PWGCF/FEMTOSCOPY/AliFemto");
    gInterpreter->AddIncludePath("$ALICE_ROOT/PWGCF/FEMTOSCOPY/AliFemtoUser");
    ...
    //
    gROOT->LoadMacro("myAliFemtoV0TrackCut.cxx+g");  //I put this line in ConfigFemtoAnalysis_K0Lam2.C to try to run on grid
...
//        alienHandler->SetRunMode("full");
        alienHandler->SetRunMode("terminate"); // Set to Terminate to merge the results of a 'full' run

        alienHandler->SetAPIVersion("V1.1x");
        alienHandler->SetROOTVersion("v5-34-08");
        alienHandler->SetAliROOTVersion("vAN-20140923");

        alienHandler->SetAnalysisSource("myAliFemtoV0TrackCut.cxx myAliFemtoKStarCorrFctn.cxx KStarCF.cxx myAliFemtoVertexMultAnalysis.cxx"); // Add any cxx files which need compiled here (.cxx files)
        alienHandler->SetAdditionalLibs(AdditionalLibs); // Add any files which need copied to grid here (.h,.cxx,.C files)

        alienHandler->AddIncludePath("$ALICE_BUILD/include");
        alienHandler->AddIncludePath("$ALICE_ROOT/PWGCF/FEMTOSCOPY/AliFemto/");
        alienHandler->AddIncludePath("$ALICE_ROOT/PWGCF/FEMTOSCOPY/AliFemtoUser/");
        ...
        alienHandler->SetRunPrefix("000"); // "000" for real (non-simulated) data
