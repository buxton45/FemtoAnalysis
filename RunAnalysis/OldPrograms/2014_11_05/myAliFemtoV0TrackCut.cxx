...
fInvMassLambdaMin(0),fInvMassLambdaMax(99),fInvMassK0ShortMin(0),fInvMassK0ShortMax(99),fMinDcaDaughterPosToVert(0),fMinDcaDaughterNegToVert(0),fMaxDcaV0Daughters(99),fMaxDcaV0(99),fMaxDecayLength(99), fMaxCosPointingAngle(0), fParticleType(99), fEta(0.8), fPtMin(0), fPtMax(100), fOnFlyStatus(kFALSE), fMaxEtaDaughters(100), fTPCNclsDaughters(0), fNdofDaughters(10), fStatusDaughters(0), fPtMinPosDaughter(0), fPtMaxPosDaughter(99), fPtMinNegDaughter(0), fPtMaxNegDaughter(99),fMinAvgSepDaughters(0), fRemoveMisidentified(kFALSE), fInvMassMisidentifiedMin(0), fInvMassMisidentifiedMax(99), fMisIDHisto(0), fCalculatePurity(kFALSE), fPurityInvMassMin(0), fPurityInvMassMax(99), fPurity(0)
{
  // Default constructor
 }
 //------------------------------
...
bool myAliFemtoV0TrackCut::Pass(const AliFemtoV0* aV0)
{
  ...
  if (fParticleType == 0 ) 
  {
      ...
      if (IsPionNSigma(aV0->PtNeg(), aV0->NegNSigmaTPCPi(), aV0->NegNSigmaTOFPi())) //pi-
	{
	  ...
	  if(fCalculatePurity)
            {
	      if(aV0->MassLambda()<fPurityInvMassMin || aV0->MassLambda()>fPurityInvMassMax) return false;  //loose InvMass cut
  ...
  else if (fParticleType == 1) 
  {
      ...
      if (IsPionNSigma(aV0->PtPos(), aV0->PosNSigmaTPCPi(), aV0->PosNSigmaTOFPi())) //pi+
	{
	  ...
	  if(fCalculatePurity)
            {
	      if(aV0->MassAntiLambda()<fPurityInvMassMin || aV0->MassAntiLambda()>fPurityInvMassMax) return false;  //loose InvMass cut
	      if(fRemoveMisidentified)
		{
		  if(aV0->MassK0Short()>=fInvMassMisidentifiedMin && aV0->MassK0Short()<=fInvMassMisidentifiedMax)
		    {
	      ...
	      fPurity->Fill(aV0->MassAntiLambda());
  ...
  else if (fParticleType == 2 ) 
  {
      ...
      if (IsPionNSigma(aV0->PtNeg(), aV0->NegNSigmaTPCPi(), aV0->NegNSigmaTOFPi())) //pi-
	{
	  ...
	  if(fCalculatePurity)
            {
	      if(aV0->MassK0Short()<fPurityInvMassMin || aV0->MassK0Short()>fPurityInvMassMax) return false;  //loose InvMass cut
	      if(fRemoveMisidentified)
		{
...
void myAliFemtoV0TrackCut::SetMisIDHisto(char* title, const int& nbins, const float& aInvMassMin, const float& aInvMassMax)
{
...
void myAliFemtoV0TrackCut::SetPurityRange(double aInvMassMin, double aInvMassMax)
{
  fPurityInvMassMin = aInvMassMin;
  fPurityInvMassMax = aInvMassMax;
}

void myAliFemtoV0TrackCut::SetPurityHisto(char* title, const int& nbins, const float& aInvMassMin, const float& aInvMassMax)
{
