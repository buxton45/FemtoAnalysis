...
//  int idx = threadIdx.x + blockIdx.x*blockDim.x;
  ...
  g_odataReal[tPairNumber] = cuCreal(tExpTermCmplx);
  g_odataImag[tPairNumber] = cuCimag(tExpTermCmplx);
}
...
//  int idx = threadIdx.x + blockIdx.x*blockDim.x;
  ...
  g_odataReal[tPairNumber] = cuCreal(tGTildeCmplx);
  g_odataImag[tPairNumber] = cuCimag(tGTildeCmplx);
}
...
//  int idx = threadIdx.x + blockIdx.x*blockDim.x;
  ...
  g_odataReal[tPairNumber] = cuCreal(tHyperGeo1F1Cmplx);
  g_odataImag[tPairNumber] = cuCimag(tHyperGeo1F1Cmplx);
}


//________________________________________________________________________________________________________________
__global__ void GetAllScattLenCmplx(double aReF0, double aImF0, double aD0, double *g_odataReal, double *g_odataImag, int aAnalysisNumber, int aBinKNumber, int aOffsetOutput)
{
//  int idx = threadIdx.x + blockIdx.x*blockDim.x;
...
*/
  ...
  int tSizeOutputBig = fNThreadsPerBlock*fNBlocks*sizeof(double);
...
*/
    ...
    GetAllScattLenCmplx<<<fNBlocks,fNThreadsPerBlock,tSizeShared,tStreams[4]>>>(aReF0, aImF0, aD0, h_ScattLenReal, h_ScattLenImag, aAnalysisNumber, i, tOffsetOutput);
