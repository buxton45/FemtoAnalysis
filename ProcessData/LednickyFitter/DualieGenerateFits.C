...
//-----------------------------------------------------------------------------
  ...
  tSaveNameModifier += TString("_Dualie");
  if(tDualieShareLambda) tSaveNameModifier += TString("_ShareLam");
  if(tDualieShareRadii) tSaveNameModifier += TString("_ShareRadii");

//-----------------------------------------------------------------------------

  DualieFitGenerator* tDualie = new DualieFitGenerator(tFileLocationBase,tFileLocationBaseMC,tAnType, tCentType,tAnRunType,tNPartialAnalysis,tGenType,tShareLambdaParams,tAllShareSingleLambdaParam);
...
//-------------------------------------------------------------------------------
  if(bDoFit)
  {
    tDualie->DoFit(true, true, tMaxFitKStar);
