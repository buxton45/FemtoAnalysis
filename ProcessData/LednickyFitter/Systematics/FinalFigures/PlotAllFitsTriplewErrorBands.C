...
//________________________________________________________________________________________________________________
td2dVec FormAllPossibleCombos(td2dVec &aVec)
{
  cout << "Trying fancy new looping" << endl;
  ...
  cout << "------------------------------" << endl;
  for(unsigned int i=0; i<aVec.size(); i++) cout << *it[i] << endl;
  td1dVec tNewCombo(0);
  for(unsigned int i=0; i<aVec.size(); i++) 
  {
...
/*  
  cout << "tParsAndErrs_LamK0.size() = " << tParsAndErrs_LamK0.size() << endl;
  cout << "tParsAndErrs_LamK0[0].size() = " << tParsAndErrs_LamK0[0].size() << endl;
  cout << "tParsAndErrs_LamK0[0][0].size() = " << tParsAndErrs_LamK0[0][0].size() << endl;
  cout << "tParsAndErrs_LamK0[0][1].size() = " << tParsAndErrs_LamK0[0][1].size() << endl;
  cout << "tParsAndErrs_LamK0[0][2].size() = " << tParsAndErrs_LamK0[0][2].size() << endl;
  
  td2dVec tParsAndErrs_LamK0_0010 = tParsAndErrs_LamK0[0];
  td2dVec tParsExtreme(tParsAndErrs_LamK0_0010[0].size());
  for(unsigned int iParam=0; iParam<tParsAndErrs_LamK0_0010[0].size(); iParam++)
  {
    double tParamVal =  tParsAndErrs_LamK0_0010[0][iParam];
    double tParamStat = tParsAndErrs_LamK0_0010[1][iParam];
    double tParamSyst = tParsAndErrs_LamK0_0010[2][iParam];
    
    double tMinExtreme = tParamVal - tParamStat - tParamSyst;
    double tMaxExtreme = tParamVal + tParamStat + tParamSyst;
    
    tParsExtreme[iParam] = td1dVec{tMinExtreme, tMaxExtreme};
    
    cout << "iParam = " << iParam << endl;
    cout << "tMinExtreme = " << tMinExtreme << endl;
    cout << "tParamVal = " << tParamVal << endl;
    cout << "tMaxExtreme = " << tMaxExtreme << endl;
    cout << endl;
    
  }
  
  td2dVec tCombos = FormAllPossibleCombos(tParsExtreme);
  cout << endl << endl << endl;
  for(unsigned int i=0; i<tCombos.size(); i++)
  {
    cout << "**********************************" << endl;
    for(unsigned int j=0; j<tCombos[i].size(); j++) cout << tCombos[i][j] << endl;
  }
*/
...
//-------------------------------------------------------------------------------
  ...
  tTriple->BuildFitErrBands_FINALRESULTSONLY();


  TCanvas* tKStarwFitsCan_CombineConj_AllAn_Zoom = tTriple->DrawKStarCfswFits_CombineConj_AllAn(ApplyMomResCorrection, ApplyNonFlatBackgroundCorrection, tNonFlatBgdFitTypes, SaveImages, bDrawSysErrs, true, tSuppressFitInfoOutput, tLabelLines, true);
