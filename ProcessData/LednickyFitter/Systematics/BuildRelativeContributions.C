...
//----------------------------------------------------------------------
td4dVec BuildRelativeContributions(td4dVec &aAll, td4dVec &aFitSys, td4dVec &aFinalVecCutSys, td4dVec &aFinalVecFitSys)
{
  //aAll[aAnType][aCentType][aParamType];
  double tNVals1 = aAll[0][0][0].size();
  double tNCuts = tNVals1/3.;  
  double tNVals2 = 10.;  ////10 because dumb NonFemtoBgd has 4 instead of 3 variations, together with 3's from FitRange and Residuals
  
  td4dVec tReturnVec(0);
  tReturnVec.resize(aAll.size(), td3dVec(aAll[0].size(), td2dVec(aAll[0][0].size(), td1dVec(tNCuts, 0))));  
  ...
  for(unsigned int iAnType=0; iAnType<aAll.size(); iAnType++)
  {
    for(unsigned int iCentType=0; iCentType<aAll[iAnType].size(); iCentType++)
    {
      for(unsigned int iParamType=0; iParamType<aAll[iAnType][iCentType].size(); iParamType++)
      {
        double tPartialVar = 0.;
        double tRelContr = 0.;
        assert(aAll[iAnType][iCentType][iParamType].size()==tNVals1);
        assert(aAll[iAnType][iCentType][iParamType].size() %3 == 0);
        for(unsigned int iVal=0; iVal<aAll[iAnType][iCentType][iParamType].size(); iVal++)
        {
          if(iVal%3==0) tPartialVar=0.;
          tPartialVar += pow((aAll[iAnType][iCentType][iParamType][iVal] - aFinalVecCutSys[iAnType][iCentType][iParamType][0]), 2);
          if(iVal%3==2)
          {
            double tDenom = aFinalVecCutSys[iAnType][iCentType][iParamType][1]*aFinalVecCutSys[iAnType][iCentType][iParamType][1]*tNVals1 +
                            aFinalVecFitSys[iAnType][iCentType][iParamType][1]*aFinalVecFitSys[iAnType][iCentType][iParamType][1]*tNVals2;
            tRelContr = 100*tPartialVar/tDenom;
            int tIdx = iVal/3.;
            tReturnVec[iAnType][iCentType][iParamType][tIdx] = tRelContr;
          }
        }
      }
    }
  ...
  for(unsigned int iAnType=0; iAnType<aFitSys.size(); iAnType++)
  {
    for(unsigned int iCentType=0; iCentType<aFitSys[iAnType].size(); iCentType++)
    {
      for(unsigned int iParamType=0; iParamType<aFitSys[iAnType][iCentType].size(); iParamType++)
      {
        double tPartialVar = 0.;
        double tRelContr = 0.;      
        assert(aFitSys[iAnType][iCentType][iParamType].size()==10);
        for(unsigned int iVal=0; iVal<aFitSys[iAnType][iCentType][iParamType].size(); iVal++)
        {
          if(iVal==0 || iVal==3 || iVal==7) tPartialVar=0.;
          tPartialVar += pow((aFitSys[iAnType][iCentType][iParamType][iVal] - aFinalVecFitSys[iAnType][iCentType][iParamType][0]), 2);
          if(iVal==2 || iVal==6 || iVal==9)
          {
            double tDenom = aFinalVecCutSys[iAnType][iCentType][iParamType][1]*aFinalVecCutSys[iAnType][iCentType][iParamType][1]*tNVals1 +
                            aFinalVecFitSys[iAnType][iCentType][iParamType][1]*aFinalVecFitSys[iAnType][iCentType][iParamType][1]*tNVals2;
            tRelContr = 100*tPartialVar/tDenom;
            tReturnVec[iAnType][iCentType][iParamType].push_back(tRelContr);      
          }
        }     
      }
    }
  ...
  //--------------------------------------
  for(unsigned int iAnType=0; iAnType<tReturnVec.size(); iAnType++)
  {
    for(unsigned int iCentType=0; iCentType<tReturnVec[iAnType].size(); iCentType++)
    {
      for(unsigned int iParamType=0; iParamType<tReturnVec[iAnType][iCentType].size(); iParamType++)
      {
        cout << "====================================================================================" << endl;
        cout << "AnType    = " << cAnalysisBaseTags[iAnType] << endl;
        cout << "CentType  = " << cCentralityTags[iCentType] << endl;
        cout << "ParamType = " << cParameterNames[iParamType] << endl;
        cout << "--------------------------------------------------" << endl;
        double tTotalPct = 0.;
        for(unsigned int iCut=0; iCut<tReturnVec[iAnType][iCentType][iParamType].size(); iCut++)
        {
          cout << "iCut      = " << iCut << endl;
          cout << "Percentage = " << tReturnVec[iAnType][iCentType][iParamType][iCut] << endl;
          cout << "--------------------------------------------------" << endl << endl;
          tTotalPct+=tReturnVec[iAnType][iCentType][iParamType][iCut];
        }
        cout << "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << endl;
        cout << "tTotalPct = " << tTotalPct << endl;
        cout << "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << endl;
      } 
    } 
  }      
...
/*
//----------------------------------------------------------------------
td4dVec BuildRelativeContributions(td4dVec &aAll, td4dVec &aFinalVec)
{
  //aAll[aAnType][aCentType][aParamType];
  double tNVals = aAll[0][0][0].size();
  int tNCuts = tNVals/3.;  
  cout << "tNVals = " << tNVals << endl;  //Should be 82
  cout << "tNCuts = " << tNCuts << endl;  //Should be 27
  ...
  for(unsigned int iAnType=0; iAnType<aAll.size(); iAnType++)
  {
    for(unsigned int iCentType=0; iCentType<aAll[iAnType].size(); iCentType++)
    {
      for(unsigned int iParamType=0; iParamType<aAll[iAnType][iCentType].size(); iParamType++)
      {
        double tPartialVar = 0.;
        double tRelContr = 0.;
        assert(aAll[iAnType][iCentType][iParamType].size()==tNVals);
        assert((aAll[iAnType][iCentType][iParamType].size()-10) %3 == 0);  //-10 because dumb NonFemtoBgd has 4 instead of 3 variations
        for(unsigned int iVal=0; iVal<aAll[iAnType][iCentType][iParamType].size(); iVal++)
        {
          if(iVal < aAll[iAnType][iCentType][iParamType].size()-10)  //Everything normal, all have 3 variations
          {
            if(iVal%3==0) tPartialVar=0.;
            tPartialVar += pow((aAll[iAnType][iCentType][iParamType][iVal] - aFinalVec[iAnType][iCentType][iParamType][0]), 2);
            if(iVal%3==2)
            {
              tRelContr = 100*tPartialVar/(aFinalVec[iAnType][iCentType][iParamType][1]*aFinalVec[iAnType][iCentType][iParamType][1]*tNVals);
              int tIdx = iVal/3.;
              tReturnVec[iAnType][iCentType][iParamType][tIdx] = tRelContr;
            }
          }
          else //Different because stupid NonFlatBgd has 4 variations
          {
            if(iVal==72 || iVal==75 || iVal==79) tPartialVar=0.;
            tPartialVar += pow((aAll[iAnType][iCentType][iParamType][iVal] - aFinalVec[iAnType][iCentType][iParamType][0]), 2);
            if(iVal==74 || iVal==78 || iVal==81)
            {
              int tIdx;
              if     (iVal==74) tIdx = 24;
              else if(iVal==78) tIdx = 25;
              else if(iVal==81) tIdx = 26;
              else assert(0);
              tRelContr = 100*tPartialVar/(aFinalVec[iAnType][iCentType][iParamType][1]*aFinalVec[iAnType][iCentType][iParamType][1]*tNVals);
              tReturnVec[iAnType][iCentType][iParamType][tIdx] = tRelContr;      
            }          
...
//-----------------------------------------------------------------------------
  ...
  if(bIncludeFitRangeSys)
  {
    ...
    td4dVec tRelContrs = BuildRelativeContributions(tAllCutSys, tAllFitSys, tFinalCutSysVec, tFinalFitSysVec);    
  }
...
/*
  ...
  ReadFitRangeAndNonFlatBgdSys(tResultsDirectory, tAllCutSys, kLamKchP, tCentralityType, tFitInfoTString, bRunOldQMNaming, bIncludeVaryResPrimMaxDecayType);
  ReadFitRangeAndNonFlatBgdSys(tResultsDirectory, tAllCutSys, kLamKchM, tCentralityType, tFitInfoTString, bRunOldQMNaming, bIncludeVaryResPrimMaxDecayType);
  ReadFitRangeAndNonFlatBgdSys(tResultsDirectory, tAllCutSys, kLamK0, tCentralityType, tFitInfoTString, bRunOldQMNaming, bIncludeVaryResPrimMaxDecayType);  

  //---------------------------------------------------------------------------------------
  td4dVec tFinalCutSysVec = ReduceCutsVector(tAllCutSys);
  td4dVec tFinalVec = tFinalCutSysVec;
  //-----------------------------------------------------------------------------------------
  td4dVec tRelContrs = BuildRelativeContributions(tAllCutSys, tFinalVec);    
