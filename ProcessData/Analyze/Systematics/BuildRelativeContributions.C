...
//--------------------------------------------------------------------------------
td2dVec BuildRelativeContributions(td2dVec &aTotalVec, td1dVec &aErrorsVec, td1dVec &aAvgsVec)
{
  double tNVals = aTotalVec[0].size();
  double tNCuts = tNVals/3.;
cout << "tNCuts = " << tNCuts << endl;  
  td2dVec tReturnVec(tNCuts);
  for(unsigned int iBin=0; iBin<aTotalVec.size(); iBin++)
  {
    assert(aTotalVec[iBin].size()==tNVals);
    assert(aTotalVec[iBin].size() % 3 == 0);
    double tPartialVar = 0.;
    double tRelContr = 0.;
    for(unsigned int iVal=0; iVal<tNVals; iVal++)
    {
      if(iVal%3==0) tPartialVar=0.;
      tPartialVar += pow((aTotalVec[iBin][iVal]-aAvgsVec[iBin]), 2);
      if(iVal%3==2)
      {
        tRelContr = 100*tPartialVar/(aErrorsVec[iBin]*aErrorsVec[iBin]*tNVals);
        int tIdx = iVal/3.;  //TODO TODO -1? I don't think so, because 2/3 rounds down to 0, 5/3 to one, etc.
        tReturnVec[tIdx].push_back(tRelContr);
      }
    }
  }
  
  double tAvg=0.;
  for(unsigned int iVar=0; iVar<tReturnVec.size(); iVar++)
  {
    cout << "iVar = " << iVar << endl;
    tAvg=0.;
    for(unsigned int iBin=0; iBin<30; iBin++)
    {
      tAvg += tReturnVec[iVar][iBin];
      cout << "iBin = " << iBin << " : tReturnVec[iVar][iBin] = " << tReturnVec[iVar][iBin] << endl;
    }
    tAvg /= 30;
    cout << endl << "tAvg = " << tAvg << endl;
    cout << "------------------------------------" << endl << endl << endl;
  }
...
//-----------------------------------------------------------------------------
  ...
  td2dVec tRelContrs = BuildRelativeContributions(tAllCfValues,tErrorsVec,tAvgsVec);
