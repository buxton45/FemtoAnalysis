...
Charged kaons are identified using the AliFemtoESDTrackCutNSigmaFilter class.  The specific cuts used in this analysis are as follows:
 ...
 \begin{itemize} 
  ...
  \begin{itemize}
   \itemsep0em
   \item $p <$ 0.4 GeV/\textit{c}: N$_{\sigma \mathrm{K,TPC}} <$ 2
   \item 0.4 $< p <$ 0.45 GeV/\textit{c}: N$_{\sigma \mathrm{K,TPC}} <$ 1
   \item 0.45 $< p <$ 0.8 GeV/\textit{c}: N$_{\sigma \mathrm{K,TPC}} <$ 3 \& N$_{\sigma \mathrm{K,TOF}} <$ 2
   \item 0.8 $< p <$ 1.0 GeV/\textit{c}: N$_{\sigma \mathrm{K,TPC}} <$ 3 \& N$_{\sigma \mathrm{K,TOF}} <$ 1.5
   \item $p >$ 1.0 GeV/\textit{c}: N$_{\sigma \mathrm{K,TPC}} <$ 3 \& N$_{\sigma \mathrm{K,TOF}} <$ 1    
  \end{itemize}
  \item Electron Rejection: Reject if N$_{\sigma e^{-},\mathrm{TPC}} < $ 3
  \item Pion Rejection:  Reject if: 
  \begin{itemize}
   \itemsep0em
   \item $p <$ 0.65 GeV/\textit{c}
   \begin{itemize}
    \itemsep0em
    \item if TOF and TPC available: N$_{\sigma \pi,\mathrm{TPC}} <$ 3 \& N$_{\sigma \pi,\mathrm{TOF}} <$ 3
    \item else
    \begin{itemize}
     \itemsep0em
     \item $p <$ 0.5 GeV/\textit{c}: N$_{\sigma \pi,\mathrm{TPC}} <$ 3
     \item 0.5 $< p <$ 0.65 GeV/\textit{c}: N$_{\sigma \pi,\mathrm{TPC}} <$ 2
    \end{itemize}
   \end{itemize}
   \item 0.65 $< p <$ 1.5 GeV/\textit{c}: N$_{\sigma \pi,\mathrm{TPC}} <$ 5 \& N$_{\sigma \pi,\mathrm{TOF}} <$ 3
   \item $p >$ 1.5 GeV/\textit{c}: N$_{\sigma \pi,\mathrm{TPC}} <$ 5 \& N$_{\sigma \pi,\mathrm{TOF}} <$ 2  
  \end{itemize} 
 \end{itemize}
